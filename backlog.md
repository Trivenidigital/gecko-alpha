...
**Files (planned):** `scout/audit/__init__.py`, `scout/audit/snapshot.py`, `scripts/gecko_audit_snapshot.py`, `scripts/gecko-audit-snapshot.sh`, `scripts/gecko-audit-snapshot-watchdog.sh`, `systemd/gecko-audit-snapshot.{service,timer}`, `systemd/gecko-audit-snapshot-watchdog.{service,timer}`, `scout/db.py` (migration `_migrate_audit_volume_snapshot_phase_b` at schema_version 20260518, wired into `initialize()` line 105), `tests/audit/`.
**Why:** `volume_history_cg` is rolling-pruned at 7 days (`scout/spikes/detector.py:55-57`). D+14 audit at 2026-05-24 needs `[detected_at, detected_at + 48h]` forward-outcome data for 2026-05-10 detections, which gets pruned 2026-05-17 — 7 days before the audit runs. Without snapshot infra, the locked §4.1 bootstrap-CI gate has no data source. Discovered via §9c discipline applied to audit setup: original brief said "verify price_cache coverage"; `price_cache` is current-snapshot-only with no history → fifth instance of lever-vs-data-path pattern in the same arc.
**Drift verdict:** NET-NEW. No existing snapshot/backup-style job exists in `scripts/` (only `backtest_*.py`, `bl060_threshold_audit.py`). Existing `volume_history_cg` writer is in `scout/spikes/detector.py`. systemd pattern mirrors deployed `systemd/gecko-backup.{service,timer}` exactly. Watchdog mirrors deployed `scripts/gecko-backup-watchdog.sh` (curl-direct-to-Telegram per R6 CRITICAL on scout.alerter).
**Hermes verdict:** none across 5 domains (SQLite snapshot, audit persistence, systemd timers, heartbeat-file watchdog, Telegram bot API from bash). WebFetch verified 2026-05-11 against `hermes-agent.nousresearch.com/docs/skills` (687 skills, none applicable to infrastructure-layer concerns).
**Effect:** Daily 04:00 UTC snapshot copies `volume_history_cg` rows for slow_burn-detected coin_ids in soak window 2026-05-10 → 2026-05-25 into non-pruned `audit_volume_snapshot_phase_b` table. UNIQUE (coin_id, recorded_at) → ON CONFLICT DO NOTHING idempotent. Per-chunk commit (CHUNK=500) keeps SQLite write-lock holds <1s — does not stall pipeline writes. Hard gate at <10G free disk space (both bash wrapper pre-run and Python pre-INSERT); abort + direct-curl Telegram alert on gate failure. Watchdog at 10:00 UTC catches heartbeat-file staleness (30h threshold) and fires Telegram alert via the gecko-backup-watchdog pattern. Final scheduled run 2026-05-25 (D+15); operator disables timer per `tasks/deployed_audit_snapshot_2026_05_11.md` (created at deploy time, operator-action block at top of file).
**Risks:**
- Disk pressure (VPS at 91% used at plan time per `feedback_vps_backup_rotation.md` history). Mitigated by hard 10G gate + abort.
- Pipeline restart for migration disrupts in-flight MiroFish/paper-trade/M1.5c state. Mitigated by deploy timing post-M1.5c soak (post-2026-05-12T02:00Z).
- Bootstrap timing: if delayed past 2026-05-17, earliest 2026-05-10 detection data is pruned. Mitigated by Task 7 sequencing bootstrap immediately after install + Step 6.5 expected-vs-actual coverage verification.
**Pre-registration discipline:** D+14 evaluation criteria locked 2026-05-11 in brief §2 (bootstrap-CI gate) + §3 (four-quadrant interpretation framework). Do NOT relax thresholds at D+14. Soak extension triggered ONLY by sample size <35, never by data-quality concerns at audit time (§11 of brief).
**Cross-references:** `tasks/findings_silent_failure_audit_2026_05_11.md` §4 Priority 6 (parallel table-freshness watchdog daemon, deferred; this snapshot's table will be on its monitored-tables list when daemon ships — defense-in-depth against stacked-failure mode).
**Estimate:** ~4-5 hours coding + tests (TDD per project convention; 9 tasks in plan covering schema, snapshot module, CLI, systemd, watchdog, brief, deploy, cross-reference, backlog), ~1 hour reviewer dispatch + fix cycles (3-vector PR review per pipeline), ~30 min PR + merge + deploy. Plan + brief + design-review already complete (R1/R2 + Q2 baked in); execution pending.
