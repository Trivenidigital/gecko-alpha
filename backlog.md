...
**Operator decisions 2026-06-30 — High-Precision Trade-Now Lane (resolves the open "scarcity algorithm / daily budget / corpus scope" gates above; full note: `tasks/product_note_trade_now_lane_2026_06_30.md`):** The product is a *separate, tiny, high-trust* TG lane — NOT making `Trade Inbox act_now` louder (honors the anti-scope → separate `/api/trade_alert_intent`-style surface). Decided params: (1) **daily budget = hard cap 1-3 pings/day** (>3 = "another dashboard queue"); (2) **strongly precision over recall** — "rare, worth opening immediately," OK to miss winners; (3) **manual trading only**, each ping carries token/chain/contract, entry mcap, age, score components, DEX source, why-it-passed, what-would-invalidate, liquidity/volume/buy-pressure, dashboard-detail link; (4) **TG-first**, later dashboard "Act Now" mirror must show *exactly* the pushed set (no broader queue). **NEW UNLOCK GATE — binds to the DEX-instrumentation soak (PRs #383-388, deployed+enabled 2026-06-29):** BLOCKED until DEX measurable cohort `n >= 30` with `>= 1` ran->=10x candidate, F1 cohort re-run with survivorship bound, AND candidate volume + false-positive cost known (a precision-capped lane needs the FP cost to set its bar; we lack the denominator until the soak delivers it — see `findings_same_asset_under_gate_cohort_30d_2026_06_28.md`). Until then: conceptual/docs only — no gate recalibration, threshold/scoring/trading-alert/proxy/paid-feed changes. Reviewer note: the right product is NOT "make act_now louder."
