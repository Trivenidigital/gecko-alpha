...
**Status:** PR-OPEN 2026-05-18 — gate fired (204 rows in `narrative_alerts_inbound` vs ≥10 threshold). `scout/api/internal_alert.py` adds POST `/api/internal/operator-alert`, HMAC-authed via the shared `_verify_hmac` from `scout/api/narrative.py` (reuse — no duplicate hardening), calls `scout.alerter.send_telegram_message(parse_mode=None, raise_on_failure=True)` with §12b dispatched / delivered / failed log triplet. Router wired into `dashboard/api.py` with the same stub-503 pattern as the narrative router. Tests in `tests/test_internal_alert_api.py` cover: feature-gate 503, missing-headers 401, bad-sig 403, replay 409, invalid-payload 400, delivery success 200, delivery failure 502, log-triplet ordering, and four secret-leakage scans across success/auth-fail/delivery-fail/disabled paths. Original PROPOSED 2026-05-13.
...
- Smoke-test shape: trigger a deliberate misconfig on the Hermes side (e.g., temporarily unset `NARRATIVE_SCANNER_HMAC_SECRET` on the gecko-alpha side, restart, then restore) and confirm a Telegram alert lands; OR have the dispatcher emit a one-off `dispatcher_self_test` POST with a known marker source.
