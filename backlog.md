...
16. **BL-NEW-CHECKER-ALERT-TEXT-UNPINNED** — PROPOSED 2026-08-24, from the concurrency slot's checker sweep on PR #560. **Six remaining unpinned strings in the alert body the watchdog forwards VERBATIM to Telegram** (`text="…result=$result"`, `parse_mode=` empty). The two highest-value ones were closed in #560 — `ALSO COLLAPSED on` (the second alarm when dark and collapsed both fire) and the `History may have been deleted…` remediation. Still unpinned: `chain_identity_recompute_v1 is MISSING while` (header of the hard-broken case), `keep their credit, far below the recorded high-water rate (` (the COLLAPSED alert's numbers), `pre-cutover chains detections keep their credit,` (the healthy line's body), `no pre-cutover chains credit to recover (` (the zero-population all-clear), and the `query failed:` / `: file does not exist` error paths. **Same "decision pinned, words not" shape** closed twice already in this tranche — the exit code and the branching are pinned, the sentence an operator reads at 3am is not. **Method note for whoever takes it:** the reviewer nearly filed two FALSE kills here by replacing whole `print()` calls, because two of those calls bundle a pinned fragment with an unpinned one — the kill came from the pinned half. Mutate ONE FRAGMENT AT A TIME, and prove the mutant is on disk before judging the result.
