...
**Current status:** FOLLOW-UP PR-READY 2026-05-15 on `codex/cg-throttle-fix` - PR #129 spacing/jitter shipped and was deployed, but runtime throttles persisted after conservative VPS tuning (`COINGECKO_RATE_LIMIT_PER_MIN=6`, `COINGECKO_MIN_REQUEST_INTERVAL_SEC=8.0`, `COINGECKO_REQUEST_JITTER_SEC=2.0`). Thirty-minute logs still showed `cg_429_backoff=131`, proving the first fix addressed burst shape but not retry amplification. Root cause: `_get_with_backoff()` retried each 429 up to four times inside the same cycle, and Telegram social resolver CoinGecko calls bypassed the shared limiter. Follow-up fix adds `COINGECKO_429_COOLDOWN_SEC=120.0`, makes CoinGecko 429 fail soft after one provider request while tripping global cooldown, threads the default cooldown through the shared limiter, and makes resolver/second-wave CoinGecko paths report 429s into the same limiter. Design updated: `tasks/design_bl_new_cg_rate_limiter_burst_profile.md`. Verification: targeted throttle suite -> 60 passed; adjacent CoinGecko/social/second-wave suite -> 159 passed.
**Action:** merge/deploy follow-up, then compare post-restart `cg_429_backoff`, `rate_limiter_global_backoff`, `resolver_transient`, and cycle intervals against the 30-minute pre-fix window. If throttles persist with retry amplification removed, next investigation is provider identity/keying: configure a CoinGecko Demo API key/header or reduce optional CoinGecko lanes before reducing scanner breadth.
