...
**Current status:** SECOND FOLLOW-UP PR-READY 2026-05-15 on `codex/cg-throttle-serialize` - PR #129 spacing/jitter shipped and PR #130 removed in-call retry amplification, but post-PR #130 logs still showed same-cycle sibling requests queued behind the limiter after the first 429. Root causes now covered: (1) `_get_with_backoff()` no longer retries each 429 up to four times; (2) Telegram social resolver and second-wave paths report 429s into the shared limiter; (3) `RateLimiter.is_backing_off()` lets CoinGecko lanes stop remaining same-cycle strategy/page fan-out after a 429 cooldown is active. This preserves already-fetched rows but prevents queued sibling requests from consuming more exhausted IP/key budget. Design updated: `tasks/design_bl_new_cg_rate_limiter_burst_profile.md`. Verification: targeted throttle suite -> 63 passed; adjacent CoinGecko/social/second-wave suite -> 162 passed.
**Action:** merge/deploy serialization follow-up, then compare post-restart `cg_429_backoff`, `rate_limiter_global_backoff`, `resolver_transient`, and cycle intervals against the 30-minute pre-fix window. If throttles persist with same-cycle fan-out stopped, next investigation is provider identity/keying: configure a CoinGecko Demo API key/header or reduce optional CoinGecko lanes before reducing scanner breadth.
