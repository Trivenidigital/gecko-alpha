...
**Status:** PR-OPEN / SCRIPT-READY 2026-05-18 — PR (TBD #158). Visibility-first fix: stale_open_count gauge + per-token persistent-stale WARN (24h dedup) + `_get_cached_price_ages` helper. Root-cause empirically confirmed as stale-source behavior (CG `/simple/price` returns empty for 21 specific tokens; 0/21 appear in other CG lanes). 27/27 tests pass on srilu Python 3.12.3. Task 4 (`/coins/{id}` fallback) descoped pending CG-rate-limit-clear + manual-curl verification. Filed `BL-NEW-HELD-POSITION-FALLBACK-COINS-ENDPOINT` + `BL-NEW-HELD-POSITION-STALE-COUNT-ALERT` as evidence-gated/baseline-first follow-ups. See `tasks/findings_held_position_refresh_rate_gap_2026_05_18.md`.
...
**Status:** PROPOSED 2026-05-18 — evidence-gated follow-up to BL-NEW-HELD-POSITION-REFRESH-RATE-GAP. Descoped from PR (TBD #158) because CG returned HTTP 429 during direct-curl verification window; cannot confirm `/coins/{id}` recovers stale tokens.
**Tag:** `held-position-refresh` `fallback` `evidence-gated` `cg-rate-limit-sensitive`
**Why:** PR (TBD #158) confirms 21 of 148 open paper_trades have stale `price_cache` because CG `/simple/price` returns no data for them. `/coins/{id}` is plausibly more complete (returns per-token detail) but unverified.
...
**Why:** PR (TBD #158) ships `stale_open_count` gauge in structured log + per-token WARN. Gauge is operator-grep-dependent. Per CLAUDE.md §12a, threshold-driven alert closes the silent-failure surface.
...
**Decision-by:** 2026-06-15 (4 weeks from PR (TBD #158) merge; baseline window must close first).
