...
- `BL-NEW-TG-ALERT-QUALIFICATION-DESIGN` - DESIGN-SHIPPED; tracker-promotion soak cleared 2026-05-29 and operator-action telemetry shipped in PR #344 (`a6dcd13a`). Next code work is `BL-NEW-TG-ALERT-INTENT-SURFACE-BUILD`, gated on scarcity/corpus decisions.
...
**Status:** DESIGN-SHIPPED / TELEMETRY-SHIPPED 2026-05-31 - tracker-to-cockpit promotion soak cleared 2026-05-29; operator-action telemetry shipped in PR #344 (`a6dcd13a`) and is deployed on srilu at master `3153fe3`.
...
**Cleared dependency:** PR #281 tracker-to-cockpit promotion remained deployed and the request-independent soak metric cleared with `>= 5` unique tracker-promoted `coin_id`s/day across mature UTC days. PR #344 added `tg_alert_operator_actions`, `/api/tg_alerts/recent`, and `/api/tg_alerts/{alert_id}/operator-action` so the operator-action/noise input is no longer structurally missing.

**Runtime verification:** 2026-05-31 live smoke on srilu confirmed `tg_alert_operator_actions` exists, schema_version `20260531|bl_tg_alert_operator_actions_v1` is applied, `/api/tg_alerts/recent?limit=3` returns HTTP 200 with `operator_action_telemetry_available=true`, and bad POSTs fail closed with 404 for unknown alert ids.

**Remaining gate:** before live alert-intent dispatch, collect operator labels through the shipped telemetry surface, decide the scarcity algorithm and target daily budget, and choose corpus scope (paper-only vs paper+tracker). If inputs are still thin, keep the next PR dashboard-only.
...
**Operator decisions 2026-06-30 — High-Precision Trade-Now Candidate Lane (resolves the open "scarcity algorithm / daily budget / corpus scope" gates above; full note: `tasks/product_note_trade_now_lane_2026_06_30.md`):** Framing — a **high-priority manual-review alert** (a candidate worth opening immediately), NOT a "trade-now signal," "buy signal," or instruction to trade. The product is a *separate, tiny, high-trust* TG lane — NOT making `Trade Inbox act_now` louder (honors the anti-scope → separate `/api/trade_alert_intent`-style surface). Decided params: (1) **daily budget = hard cap 1-3 pings/day** (>3 = "another dashboard queue"); (2) **strongly precision over recall** — "rare, worth opening immediately," OK to miss winners; (3) **manual trading only**, each ping carries token/chain/contract, entry mcap, age, score components, DEX source, why-it-passed, what-would-invalidate, liquidity/volume/buy-pressure, dashboard-detail link; (4) **TG-first**, later dashboard "Act Now" mirror must show *exactly* the pushed set (no broader queue). **NON-NEGOTIABLE UNLOCK GATE — binds to the DEX-instrumentation soak (PRs #383-388, deployed+enabled 2026-06-29):** BLOCKED until ALL hold (no partial unlock, no exceptions): (1) DEX measurable cohort `n >= 30`; (2) `>= 1` ran->=10x candidate; (3) F1 cohort re-run completed with survivorship bound; (4) candidate volume + false-positive cost known; (5) **precision floor chosen from that evidence, not placeholder intuition**. (A precision-capped lane needs the FP cost to set its bar; we lack the denominator until the soak delivers it — see `findings_same_asset_under_gate_cohort_30d_2026_06_28.md`.) Until then: conceptual/docs only — no gate recalibration, threshold/scoring/trading-alert/proxy/paid-feed changes. Reviewer note: the right product is NOT "make act_now louder."
...
**Runtime drift-check 2026-05-30 (read-only):** Required check #1 FAILS on the named host. Non-mutating SSH inspection of srilu-vps found NO Kraken MCP present — no codex/claude MCP config, no npm/pip package, no binary/dir, no running process, no `KRAKEN*` env-var names in any `.env`. In-tree drift: 0 `kraken` refs in scout/dashboard/scripts (net-new). Live-exec substrate is shadow/disabled (`scout/config.py:101-118`: `live_mode="shadow"`, `live_trading_enabled=False`); BL-055 not unlocked. **BLOCKED — operator must install/configure the Kraken MCP on srilu (or reconcile the 'scoped on srilu' premise if it lives elsewhere) before any design/build.** Note: main-vps/vpin-vps not independently verified this session due to an SSH alias-routing anomaly (all aliases resolved to srilu's multiplexed connection).
**Security audit 2026-05-30 (provenance RESOLVED — do not re-litigate):** `github.com/krakenfx/kraken-cli` is AUTHENTIC and OFFICIAL — `krakenfx` is the verified Kraken org (id 3487933, since 2013, blog kraken.com); `LICENSE` = Payward, Inc. (Kraken); 615★, MIT, Rust; confirmed by Kraken's own blog + product page. Source-audited from a clone: egress is Kraken-only (api/futures/docs.kraken.com), no third-party exfil/webhooks/hardcoded wallets, `telemetry` module = request headers to Kraken's own API only; `install.sh` performs sha256 checksum verification before install; default `.mcp.json` ships SAFE (`kraken mcp -s market,paper` — market+paper, no live keys). **The earlier PR #333 'repo 404' was a stale/transient API result; the repo is real.** Remaining risk is CONFIGURATION, not provenance: `kraken mcp -s all --allow-dangerous` bypasses per-call confirmation over 34 dangerous commands incl. withdrawals; there is NO official npm/PyPI package (registry names are abandoned/squattable — reach the binary only via a kraken.com link). **Adoption guardrails (gate any future build): never enable Funds-Withdraw on the agent key; read-only (Funds-Query + Orders-Query) first; IP-allowlist the key to srilu; key expiry + rotation; start in paper/`market,paper` mode; pin a release tag and verify checksum.** Item remains OPERATOR-GATED + BL-055-gated + MCP-not-installed-on-srilu — authenticity is cleared, but nothing is buildable until the operator installs/scopes the MCP and BL-055 unlocks.
...
### BL-NEW-MISSED-WINNER-SURFACED-JUNK-REVIEW — SHELVED 2026-05-30
**Status:** SHELVED 2026-05-30 — structural data limit — weeks-later unobservable / no persisted surface membership / levers unpersisted; revisit on persisted Today's-Focus membership OR >7d retention. Design preserved on master at `tasks/design_missed_winner_surfaced_junk_review_2026_05_30.md`. Honest finding: mostly NOT-BUILDABLE-OFFLINE — weeks-later winners unobservable (7-day retention), "surfaced" observable only for the gainers cohort (Today's Focus has no persisted membership), terminating levers (block_cause / venue) not persisted. A built version would be gainers-cohort-only with large `unobserved` counts. Must not feed live ranking without a separate no-lookahead design.
...
**UNBLOCKED 2026-06-30 — canonical resolver now exists:** the DEX-instrumentation `contract_coin_map` (I1, PRs #383-388) maps a CA -> the **canonical CoinGecko coin_id** via `/coins/{id}.platforms`. This is exactly the "canonical CA-to-CoinGecko-id resolver" the constraint required — NOT a surrogate (it is the CG id, not the CA/ticker), so writing `resolved_coin_id` from it is compliant. The narrative-resolve-observability PR (feat/narrative-resolve-observability) implements: (a) `resolve_ca` now returns `coin_id` from `contract_coin_map` (was hardcoded `None`), and (b) a one-time backfill that retro-resolves existing CA rows the map can now identify. Forward + backfill sweep both use the canonical id. **Note:** this resolves only the ~3.4% CA-bearing rows; the 97% cashtag-only population needs the separate symbol resolver below. Identity resolution != price coverage (still BL-PRICE-COVERAGE).

### BL-NEW-NARRATIVE-SYMBOL-RESOLVER: cashtag ($TICKER) -> coin_id resolution for narrative inbound
**Status:** PROPOSED 2026-06-30 — surfaced by the narrative-resolve-observability PR (feat/narrative-resolve-observability). ~97% of `narrative_alerts_inbound` rows (2,085/2,140 on 2026-06-30) are CASHTAG-ONLY (no contract address). The CA-only resolver cannot touch them; gecko-alpha has no `lookup_by_symbol(symbol, chain)` primitive. **This is the DOMINANT narrative resolve gap** — the CA-resolver fix only addresses the ~3.4% CA-bearing rows.
**Tag:** `narrative` `resolver` `symbol` `identity` `hermes-first` `precision-risk`
**Constraint (inherited from BL-NEW-HERMES-NARRATIVE-DEFERRED-RESOLUTION-SWEEP):** do NOT write `resolved_coin_id` with a surrogate (ticker/CA). Must resolve to a canonical CoinGecko coin_id or stay NULL + classified `cashtag_only`. A symbol is ambiguous; resolving it wrong is worse than leaving it unresolved.
**Why hard (design, not hack):**
- **Ambiguity / ticker collisions:** many coins share a cashtag; symbol -> coin_id is one-to-many. Needs a disambiguation policy (mcap rank / chain context / recency / source trust).
- **Chain context:** cashtag-only rows usually have no chain (2,053 empty-chain rows on 06-30); CG symbol search returns cross-chain matches.
- **False joins:** a fuzzy symbol/name match is exactly the contamination the ANSEM/CATWIFHAT work flagged (copycats $CWIF/WIF/catwif). **Do NOT add a broad fuzzy-name join path.**
- **CG lookup cost:** CG `/search` per cashtag at ingest rate burns the shared 30 req/min budget — needs caching + batching.
- **Precision risk:** wrong resolution overstates source-call rankability (same hazard the SWEEP constraint warns about).
...
**Refresh 2026-05-29:** n_actionable=92 (-$475.12 / 53W 39L / 57.6%), n_exploratory=30 (-$738.48 / 12W 18L / 40.0%). Row-count gate still cleared (now both ≥20). 2026-05-26 follow-up trigger on below-$10M `chain_completed` reached n=24 (≥20 half met) BUT bucket-turns-positive half REFUTED (-$305→-$678 net). Cohort separation compressed (30.2pp→6.5pp on avg pnl_pct) consistent with regime-window overlap (2026-05-13→2026-05-19 gainers_early autosuspend per `tasks/findings_gainers_early_autosuspend_attribution_2026_05_29.md`). No classifier-change trigger; no implementation authorized. See `tasks/findings_actionability_gate_recheck_2026_05_29.md`.
...
**Status:** DESIGN-MERGED / RE-SCOPE-ELIGIBLE-AFTER-ACTIONABILITY-REVALIDATION 2026-05-26 - design doc merged via PR #184 as `2e2f506c` (squash 2026-05-21). The actionability `20/5` row-count gate cleared in `tasks/findings_actionability_gate_revalidation_2026_05_26.md`; implementation still must start with a fresh drift/runtime re-scope and must not treat the actionability finding as automatic approval. **Refresh 2026-05-29:** n=92+30 still RE-SCOPE-ELIGIBLE; per `tasks/findings_actionability_gate_recheck_2026_05_29.md`, the 2026-05-26 follow-up trigger (below-$10M `chain_completed` bucket-turns-positive) is REFUTED by 3 more days of data. Note: PR #206 (source-call outcome ledger, `source_calls` sidecar) shipped 2026-05-22 with X heuristic trade linkage — operator drift/runtime re-scope must reconcile #184's residual `linkage_state` enum + unified view against PR #206's landed shape before scoping.
...
### BL-NEW-TODAYS-FOCUS-LIQUIDITY-VENUE-FACTS
**Status:** PHASE-1A-II-SHIPPED 2026-05-29 — Phase 1a-i (PR #324, schema + settings + watchdog) + Phase 1a-ii (this PR, cron writer + upsert preservation + static grep ban). Phase 1a-ii adds: `scripts/backfill_dexscreener_liquidity.py` (deterministic CG slug → `/coins/{id}.platforms` → DexScreener `/tokens/v1/{chain}/{address}` resolution; `dex:<chain>:<address>` prefix shortcut; per-row fail-soft; killswitch-respecting; CG-limiter coordinated; heartbeat touched on every successful tick incl. no-work); `upsert_candidate` switched from `INSERT OR REPLACE` to UPSERT (`ON CONFLICT DO UPDATE`) so the 4 enrichment columns survive re-ingest cycles (round-trip preservation); `tests/test_no_symbol_fuzzy_resolution.py` static-grep CI gate bans `/dex/search` substrings anywhere in cron path (turns operator guardrail #3 into a runtime contract per `feedback_anti_scope_as_runtime_contract.md`). No dashboard read change. No downstream consumer. **Phase 1a-i previously:** schema migration + settings + watchdog landed; cron writer absent. Decision packet `tasks/decision_liquidity_backfill_2026_05_29.md` (PR #322) selected option (b2) background cron backfill via DexScreener search. Design `tasks/design_liquidity_enrichment_b2_2026_05_29.md` (PR #323) authorized phased build. **Phase 1a-i (this PR):** `_migrate_liquidity_enrichment_v1` adds 4 nullable `candidates` columns (`liquidity_usd_enriched` REAL, `liquidity_enriched_source` TEXT, `liquidity_enriched_at` TEXT, `liquidity_enriched_confidence` TEXT) with no DEFAULT (absence-vs-zero semantics preserved). Settings: `LIQUIDITY_ENRICHMENT_ENABLED=False` killswitch, `LIQUIDITY_ENRICHMENT_TTL_SEC=1800`, `LIQUIDITY_ENRICHMENT_STALE_SEC=3600`, `LIQUIDITY_BACKFILL_BATCH_MAX=50`. Watchdog `scripts/check_liquidity_enrichment_lag.py` mirrors `check_source_calls_lag.py` shape with writer_stale / heartbeat_missing / heartbeat_pending / writer_never_fired states + killswitch-disabled short-circuit + row-rate SLO (default ≥80% recent rows fresh within 30 min over 7d window). **Phase 1a-ii (next PR):** cron writer with CG-slug → `/coins/{id}.platforms` → DexScreener `/tokens/v1/{chain}/{address}` resolution + static-grep ban on `dex/search?q=`. **Phase 1c (later):** systemd timer / crontab install. **Phase 2 (separate design):** downstream consumer; gated on pre-registered ≥70% coverage on paper-cohort denominator + accuracy ±20% on ≥80% of ≥20 samples (broader candidates set). No dashboard read change in Phase 1a-i. No downstream consumer until Phase 2 gates pass.
**Why:** Today's Focus paper cohort is structurally CG-sourced; `candidates.liquidity_usd` is 0 for 995/995 `chain="coingecko"` rows because CG `/coins/markets` Demo does not surface liquidity. Trader cannot answer "can I actually trade this" without liquidity context. Audit PR #310/#311 confirmed coverage_rate=0.0 on paper cohort.
**Scope:** Phase 1a-i = schema + settings + watchdog (this PR). Phase 1a-ii = writer. Phase 1c = scheduling. Phase 2 = downstream consumer (operator-decision-gated).
...
8. **BL-NEW-LEDGER-LABEL-THROUGHPUT-DEFICIT** — PROPOSED 2026-08-21, surfaced while measuring the lateness margin for PR #548 (`volume_history_cg` retention repair 2 of 2). **The labeler is structurally slower than emission, so its lateness ceiling is not stationary — and #548's safety margin is sized against a measurement of that ceiling.** Measured on prod 2026-08-18: ledger emission ~20–31K rows/day vs labeler completion ~7–11K rows/day. `LEDGER_LABEL_BATCH_MAX=500` per hourly pass caps throughput at 12K/day, i.e. **below the emission rate by construction**. Observed lateness past the r7d deadline (n=273,296 completed rows) was p50 0.153d / p90 2.529d / p99 2.857d / **max 2.910d**, which fits inside the 3d `LEDGER_LABEL_MAX_LATENESS_DAYS` margin #548 set — **today**. **Why it matters:** if the deficit persists, the lateness ceiling grows past 3d and re-opens exactly the silent truncation #548 closed — `outcome_ledger._peak_price_in_window` would again compute `peak7d` over a left-truncated window and return a biased value rather than NULL. The validator pins retention ≥ horizon + margin, but **nothing currently watches whether the margin is still big enough**, so this would degrade silently a second time. **Scope:** (a) §12a watchdog alerting when observed p99 lateness exceeds some fraction of `LEDGER_LABEL_MAX_LATENESS_DAYS` — the missing feedback loop, and the cheapest item here; (b) decide whether to raise `LEDGER_LABEL_BATCH_MAX`, make the batch adaptive to backlog depth, or accept the deficit and widen the margin (each has a different disk cost at ~710K vhcg rows/day); (c) re-run the lateness measurement after any change and re-derive the margin — **do not guess it**, per the 2026-08-16 retention ruling. **Note:** the 170K `pending` backlog is NOT itself the pathology — it is bounded by the 7d emission window (~24K/day × 7d) and only 150 rows were past the r7d deadline at measurement time. The deficit, not the backlog depth, is the signal to watch.

9. **BL-NEW-CACHE-PRICES-SHARED-CONNECTION-LAUNDERING** — PROPOSED 2026-08-23, surfaced by the structural/concurrency reviewer while verifying PR #559's ratchet write. **Pre-existing, not introduced by #559 — do not attribute it to that branch.** `db.cache_prices` commits on the *shared* connection, so its `commit()` also commits whatever a sibling coroutine has pending. Measured with a discriminating control (a detector that only ever says "clean" proves nothing): `cache_prices` → **1 sibling row survived rollback (LAUNDERED)**; `_record_coverage_baseline`, which #559 moved to its own connection, → **0 (clean)**. **Why it matters:** the dominant path is reached every pipeline cycle from `main.py:1006`, concurrently with `run_chain_tracker`'s bare `BEGIN`/`rollback()` which does not take `_txn_lock` — so a chain-tracker rollback can be silently converted into a partial commit. This is the same class the repo already answers with the REPAIR pattern (`reconcile_signal_first_seen` exists precisely because this ordering cannot be made atomic), so the fix shape is likely an idempotent repair + drift counter rather than an attempt at isolation; SAVEPOINT is a stack, not a scope, and cannot isolate coroutines on a shared connection. **Scope:** (a) enumerate every writer that commits on the shared connection concurrently with a bare-`BEGIN` reader — by grep, not by memory; (b) decide repair-vs-isolation per site; (c) a drift counter so the failure stops being silent. **Related residual from the same review, lower severity:** #559 made one `cache_prices` commit unconditional (the `count == 0` path), which launders identically — reachability is low because all four call sites guard with a non-empty check (`main.py:1004`, `narrative/agent.py:325`, `outcome_ledger.py:978` and `:1065`), so it needs a non-empty list in which every element lacks `id`, i.e. malformed API data. That delta does not change the character of the pre-existing defect and was accepted at merge; it closes when (a) closes.

10. **BL-NEW-RECOMPUTE-SUITE-AXIS-COVERAGE** — PROPOSED 2026-08-23, from the silent-failure reviewer's terminal verdict on PR #559 (finding F3). **No live defect; this closes a hiding place, not a hole.** `test_the_mark_RISES_when_recovery_improves` hardcodes `gainers_comparisons`, so the suite cannot see a *per-surface* freeze of the coverage ratchet. Demonstrated with a behavioural mutant (`if rate > best and source_table == "gainers_comparisons"`) — **all 116 tests pass**, while running the real deploy sequence against two surfaces at once shows `trending_comparisons` frozen at the pre-backfill `0.0` with its collapse silent and nothing red. That is the `6276d586` regression exactly, scoped to one surface, and it hides on the surface most exposed to it: trending oscillates near the floor and is least like gainers. **Fix:** parametrise over `Database._RECOMPUTE_SURFACES`, the way `tests/test_recompute_overlay_tiebreak.py:59,86,139` already parametrises its three readers. **Bundle with:** lifting the concurrency reviewer's two-layer harness and discriminating laundering control out of `ga-rev-c1a-a/.rev-a-scratch/` into `tests/` — the laundering control is more useful as a permanent regression test for BL-NEW-CACHE-PRICES-SHARED-CONNECTION-LAUNDERING than as a one-off. Deliberately NOT applied inside #559: changing `tests/` changes what CI runs, so it is not a documentation delta, and every clearance in that tranche was measured on a tree without it. **The general rule this came from, worth more than the fix:** *a reviewer's battery inherits the shape of the first defect they found* — the defence is not more scenarios along the axis you already know, it is asking which axis the suite never varies. For this component the axes are **direction, surface, layer, durability of stored state**; the suite ranges over three of the four.

11. **BL-NEW-RECOMPUTE-PROBE-OBSERVABILITY-RESIDUALS** — PROPOSED 2026-08-23, two minor items from the same verdict. Both are production changes (`scout/db.py`, `scripts/check_recompute_coverage.py`), hence deferred out of #559 rather than fixed at merge. **(a) The taxonomy computes `decision` and discards it.** `mark_written: false` covers both "deliberately did not write" (`compare`) and "tried and lost" (`raise_mark`, write starved). The payload IS decidable today — `compare` is only reachable when `rate <= best` and `raise_mark` only when `rate > best`, so `rate > best_rate` with `mark_written: false` means the write was attempted and lost, and a `recompute_coverage_baseline_write_failed` warning fires on that path. The gap is that the distinction must be *derived* from arm semantics the payload never states; emitting `decision` makes it explicit rather than inferable. **(b) `comparison_skipped` is invisible where it pages.** The probe reports `comparison_skipped='incomparable_mark_not_cleared'`; the checker computes no equivalent and prints `mark=0.8000` with `exit=0` — printing the mark actively implies a comparison happened, so "could not judge this surface" is presented identically to "judged it, healthy." The vocabulary already exists for the no-baseline case (`[collapse detection NOT ARMED]`); there is no per-surface equivalent. `mark=0.8000 (not compared)` closes it. Transient state, not a broken one.

12. **BL-NEW-CI-LAPSE-GATE** — PROPOSED 2026-08-23, from the structural/concurrency reviewer's closing correction on PR #559. **The clearance-lapse rule currently depends on the implementer choosing to enforce it against their own interest.** On #559 it worked — two slots were sent back after `scout/db.py` moved, and both re-runs produced findings that did not exist at the clearances that would otherwise have been carried forward. But the rule did not catch that; a person did, at the point where accepting an existing clearance was easiest and cheapest. That is discipline under observation with four reviewers watching, not a mechanism, and it is the failure mode the report's own §7 argues against (*prefer the version of a virtue that has a mechanism attached*). **Scope:** a CI check that, given a PR's recorded reviewer clearances (SHA per vector — the merge report's §4 table is already the machine-readable-ish source), recomputes lapse status at the PR head and fails when any clearance's `scout/` or `scripts/` tree hash differs. Must implement the *corrected* rule, not the name-level one: (1) `git fetch origin` first — a stale base ref is silent to the ancestry guard and inflated #559's footprint from 36 files to 91; (2) `git merge-base --is-ancestor` as the guard, so a non-ancestor SHA reports invalid rather than a spurious mass revert (the `origin/HEAD` spelling produced a false LAPSED reporting 3,546 deletions); (3) compare tree hashes, not `--name-only`, which misses a file restored to the wrong baseline. **Also worth encoding:** a commit existing is not a commit being reachable — the check must evaluate ancestry from the revision under review, not from whatever the author last pushed. Cousin to the existing anti-scope CI gates. **Sub-item — the SHA-extraction pattern itself is defective, and this was filed wrong once already.** Auditing the shipping docs, a `[0-9a-f]{7,10}` scan reported five MISSING commits in `docs/migration_versions.md`; all five are `schema_version` integers (`20260517`, `20260702`). I recorded that as "no defect" because the *documents* were fine — but the *checker* produced five confident false positives, and a future reader has no way to tell its output from a real finding. Fix (silent-failure reviewer): require at least one `a-f` in the token. **Verified:** that excludes all five. **Cost, stated because it is a trade not a free win:** it makes the checker silent on an all-numeric abbreviated SHA — 3.7% of 7-char and 2.3% of 8-char abbreviations. A targeted alternative with no SHA cost is to exclude only tokens that are exactly 8 digits and parse as a plausible `YYYYMMDD`, which is the actual collision class here since this repo's `schema_version` convention is date-based (see BL-NEW-MIGRATION-VERSION-SCHEME). Prefer the targeted rule; note the trade either way rather than presenting the pattern as strictly better.

13. **BL-NEW-CLEARANCE-CHECK-STRUCTURAL-RESIDUALS** — PROPOSED 2026-08-24, from four review rounds on PR #560. **Three items the PR fixed as instances rather than as shapes, plus two operator actions that cannot be done from inside the repo.** Filed together because the review's own conclusion was that another fix round was the wrong move: across four rounds, *every* fix was correct about the thing it was aimed at and widened a channel just past it — the shape block subsuming the per-vector loop, the timeout widening `RuntimeError`, the coercion guard stopping one statement short of `set(required)`, and the arming script's predicate. Fixing the fifth instance is not the same as addressing the generator. **(a) `_tree` conflates "git failed" with "path absent"** (`scripts/check_reviewer_clearances.py`). It returns `None` on any `RuntimeError`, and `None` is `_moved`'s encoding for "not present at that revision". A timeout was routed through that same channel and produced a demonstrated FAIL-OPEN — a PR that moved `scout/` reported "no delta in watched paths; no clearance required", exit 0. Closed for timeouts via a distinct `GitTimeout` type, but the *shape* remains: any future `_git` failure mode inherits the same conflation. Durable fix is a tri-state (`hash` / `absent` / `unknown`) rather than `str | None`, so "I could not tell" is unrepresentable as "not there". **RECURRED 2026-08-24 in the fix for this very item, one layer down.** `_master_declaration_text` in `tests/test_reviewer_clearance_gate.py` shipped `return got.stdout if got.returncode == 0 else None`, routing every nonzero `git show` into the `None` the caller reads as "master has no declaration" — so a corrupt object or `fatal: bad object` became a silent PASS against a master genuinely carrying stale clearances. The REF layer was split correctly and the FILE layer reproduced the conflation. Closed by determining absence POSITIVELY (`git ls-tree` exits 0 with empty output for a path that is not there, so nonzero is unambiguously "could not tell" and raises). **The lesson is the recurrence, not the instance:** fixing a fail-open teaches the shape and not the habit of looking for it one layer down. Found by the logic vector, which constructed the failing case after another vector looked for it and judged it unreachable. **(b) The merge-base anchor is sound ONLY while this repo squash-merges.** A squash applies `diff(merge-base, head)`, so change-then-revert within a branch is correctly exempt. Under true merge commits, a branch that merges master and then reverts master's change to a watched path lands that revert while `diff(merge-base, head)` can be empty — an unreviewed production change with no delta. Latent today; breaks silently on a repo setting nobody would connect to this file. Encode as a CI assertion on the merge strategy, or switch the anchor. **(c) The `initialize()` AST guard** in `tests/test_arm_recompute_coverage_mark.py` collects `n.func.attr` over `Call` nodes, so it misses `getattr(db, "initialize")()` and `f = db.initialize; f()`. Deliberately left: the realistic reintroduction is the literal call and the guard can no longer be satisfied or defeated by a comment, which was the actual defect. **(d) OPERATOR — branch protection is OFF** (`gh api .../branches/master/protection` → 404, rulesets `[]`). With none configured, `mergeStateStatus` reads `UNSTABLE` rather than `BLOCKED`, so **no CI check in this repo can block a merge**; PR #560 was mergeable with its own check red. Until this is enabled with checks marked required, every gate here is advisory text in a log. **(e) OPERATOR — there is no author-external review record.** `.reviewers.toml` is committed and author-writable, so repointing all four clearances at the head is a four-line edit that turns the check green *and* makes the tree comparison vacuous (clearance tree compared against itself). The standard fix — read commit-bound approvals from the GitHub API — has nothing to read: **zero approvals on #551, #555, #556, #557, #558, #559**, and `reviewDecision` empty on all 15 most recent merged PRs, because this project's independent reviewers are agents whose verdicts live in session transcripts. **Provenance, because this item's subject is self-attested records and it would be poor form for its own evidence to be one:** the ops-safety slot independently verified four of the six PRs (#555/#557/#558/#559) and none of the 15; the CI-gate slot established the six-PR and 15-PR claims; the author re-ran both in full on 2026-08-24 (six PRs → 0 reviews each; 15 merged PRs → `reviewDecision` empty on every one). Each half is now carried by someone who ran it. Until verdicts are recorded somewhere the author cannot write, the record is self-attested by construction and the check is a lapse *detector*, not an enforcement gate. (d) and (e) together are the only path to the mechanical enforcement originally asked for; neither is code.

14. **BL-NEW-RECOMPUTE-PROBE-LIVENESS-HEARTBEAT** (was: -BASELINE-FRESHNESS-SLO, RE-AIMED 2026-08-24) — PROPOSED 2026-08-24, from the SILENT-FAILURE vector on PR #560 (their S4 -- an earlier version of this ticket mis-credited it to the ops-safety slot, who raised no such finding; a ticket whose subject is self-attested records mis-crediting its own evidence is the same defect one level up). **§12a gap on the one table the collapse half of the alarm rests on.** `recompute_coverage_baseline` (`scout/db.py:387-392`) has a `recorded_at TEXT NOT NULL` column and **nothing reads it** — grepped every consumer: `scout/db.py` writes/clears/reads `best_rate`+`population`, `scripts/check_recompute_coverage.py` reads `best_rate`+`population`. No freshness SLO, no staleness watchdog. **Why it matters:** the probe is the table's only writer and its call site swallows every failure to journald (`scout/main.py:1817-1825`, `logger.exception`), so *"the probe has been raising every pass for a week"* is invisible on the paging layer — marks freeze at their last value, a never-armed surface stays never-armed, and (before the per-surface fix in this PR) a stale gainers row kept the NOT-ARMED notice suppressed. This is the project's own §12a rule applied to a table this tranche shipped: *every new DB table that records pipeline activity ships with a freshness SLO and a watchdog*. **RE-AIMED after the ops-safety slot's objection was checked against the code and HELD.** `_record_coverage_baseline` is called at `scout/db.py:8563` and **only** under `decision in ("rearm", "raise_mark")`. The `compare` arm -- comparable mark, today does not improve it -- is the healthy steady state and performs **no write at all**. So `recorded_at` legitimately ages indefinitely on a perfectly healthy system, and a staleness clock on it would fire continuously in the normal case. That is the cry-wolf property this PR rejected three separate times (S1b, the below-floor notice, the `mark_written` predicate); shipping it would have re-created that defect a fourth time, in the ticket written to prevent it.

**But the objection kills the INSTRUMENT, not the OBSERVATION** (silent-failure slot, conceding the instrument and keeping the finding). The real hole is that **nothing observes the probe RAN.** Its only failure path is `logger.exception` at `scout/main.py:1817-1825` -> journald -> declared operationally silent by this subsystem's own founding premise. And the no-write steady state makes the ratchet table uniquely useless as a liveness proxy: *"no recent writes" is ambiguous between healthy and "the probe has been raising every pass for a week."* That ambiguity is the reason a separate signal is needed, not a reason to drop the ticket.

**SCOPE: a probe-execution heartbeat** -- last-run timestamp with an hourly SLO -- **not** mark freshness. That is the §12a-shaped instrument, and it is unambiguous in both directions where `recorded_at` is unambiguous in neither. If the ops-safety slot disagrees with the heartbeat too, hear them out; they were right the first time.

**Superseded original reasoning, kept so the re-aim is auditable:** a freshness SLO is arguably the WRONG instrument here, because this table's healthy steady state is *not written* -- the `compare` arm writes nothing on every good re-run. A staleness clock would fire continuously in the normal case, which is the "alarm that fires on the steady state gets muted before the day it matters" property this runbook argues against in its own Monitoring section. And the failure it would nominally catch is already covered: a vanished mark surfaces per-surface as `NOT ARMED` through the watchdog on its timer -- event-driven rather than clock-driven. §12a's target is tables recording pipeline ACTIVITY, where absence of rows means the writer died; this one is a calibration mark. **Resolve this argument before implementing.** **Scope if it survives:** ~5 lines in the checker — compare `MAX(recorded_at)` against the probe's hourly period and report staleness on the paging layer, not journald. Composes with `BL-NEW-CLEARANCE-CHECK-STRUCTURAL-RESIDUALS`: both are cases where the only signal for a broken component goes to a layer this subsystem's own founding premise declares unreadable.

15. **BL-NEW-REPO-PATH-EXECUTABLE-BIT-DERIVATION** — CLOSED-IN-PR 2026-08-24 (#560), recorded because the *class* outlives the fix. Deployment is `git pull` into `/root/gecko-alpha`, and systemd units and cron lines invoke scripts **out of that checkout** (deliberately — installing a `.sh` to `/usr/local/bin` while it invokes a `.py` from the repo splits a watchdog in half, the documented backup-scripts trap). So the executable bit is a **deploy-path precondition**: ship `100644` and the unit fails on permissions, the timer fires, the watchdog never runs, and nothing reports it. **Two real instances found:** `scripts/recompute-coverage-watchdog.sh` shipped `100644` with a `chmod +x` step in its runbook (the step was on the activation path and skippable, and it left the prod checkout permanently dirty on a pure mode diff, camouflaging genuine drift); and `scripts/cron-drift-watchdog.sh` shipped `100644` while `cron/README.md:94` hands the operator a copy-paste crontab line invoking it by repo path, with no `chmod` in `cron/deploy.sh` — latent only because that line is not in the active `gecko-alpha.crontab`. Both now `100755` via `git update-index --chmod=+x`, and `tests/test_watchdog_ships_executable.py` **derives** the list by parsing `ExecStart`/`ExecStartPre` and `cron/` for repo-rooted `.sh` paths rather than hard-coding it. **Count, stated precisely because getting it wrong is the same class as the defect:** the derivation covers **16 scripts**; the file has more tests than that (parametrised plus structural). An earlier version of this entry said "19 covered", which was the *test* count — real, but not measuring the asserted thing. `cron-drift-watchdog.sh` is fixed to 100755 but deliberately NOT in the covered set: it appears only in `cron/README.md` prose, in no crontab and no unit, and letting documentation put a script in scope is the "scan satisfiable by prose" category error. If it is ever added to the crontab the derivation picks it up automatically. **Residual for a future reader — VERIFIED THEORETICAL, not live** (test-efficacy slot checked every `ExecStart`/`ExecStartPre` in the tree that lacks the prefix): seven units set `WorkingDirectory=/root/gecko-alpha` but **none** invokes a bare `.sh` relying on it; the one relative path is a `.py` run through `uv run python`, where the bit is irrelevant because the interpreter reads the file; and the `/usr/local/bin/` invocations are installed copies, correctly out of scope. So: the derivation only sees invocations spelled with the literal `/root/gecko-alpha/` prefix. A unit using a relative path, `WorkingDirectory` plus a bare name, or a different deploy root is invisible to it. That is a narrower gap than a hand-maintained literal but it is not zero.

16. **BL-NEW-CHECKER-ALERT-TEXT-UNPINNED** — PROPOSED 2026-08-24, from the concurrency slot's checker sweep on PR #560. **FIVE remaining unpinned strings in the alert body the watchdog forwards VERBATIM to Telegram** (`text="…result=$result"`, `parse_mode=` empty). The two highest-value ones were closed in #560 — `ALSO COLLAPSED on` (the second alarm when dark and collapsed both fire) and the `History may have been deleted…` remediation. **Count corrected from six to five:** a re-check found `keep their credit, far below the recorded high-water rate (` was ALSO closed by the same commit, unclaimed — it is the COLLAPSED alert's numbers and it dies with the remedy. Still unpinned: `chain_identity_recompute_v1 is MISSING while` (header of the hard-broken case — note its BODY, `rows depend on it … will raise`, IS pinned, so an operator would see the consequence without the subject); `pre-cutover chains detections keep their credit,` (healthy-line body); `no pre-cutover chains credit to recover (` (zero-population all-clear); `query failed:`; `: file does not exist`. **None is a second alarm or a remediation step**, which is the criterion that made the two closed ones worth pulling forward. A separate slot verified the deferral independently and reached the same set by a different route: a string is safe to defer iff losing it cannot change what the operator DOES — the three exit-2 diagnostics all route to the same next action (go look at the database), whereas losing a remedy sends the operator to an action that cannot work. **Same "decision pinned, words not" shape** closed twice already in this tranche — the exit code and the branching are pinned, the sentence an operator reads at 3am is not. **Method note for whoever takes it:** the reviewer nearly filed two FALSE kills here by replacing whole `print()` calls, because two of those calls bundle a pinned fragment with an unpinned one — the kill came from the pinned half. Mutate ONE FRAGMENT AT A TIME, and prove the mutant is on disk before judging the result. **THE ESCAPING TRAP IS THREE-FOR-THREE and belongs beside it, because it is the same lesson:** an anchor written with a real newline where the source has a literal `
` (or the reverse) matches nothing, the edit never applies, and the run reports SURVIVED. It has now bitten a reviewer's first sweep, the author's mutation loop, and the same reviewer again on the re-run. **The harness fails silently in the direction of NO RESULT, which reads identically to NO FINDING.** Same family as a baseline that was already red before the mutation (a result against a red baseline is meaningless — assert the baseline is clean and refuse to proceed otherwise) and as an injection that never fires (assert a counter moved). All four are the same shape: *an unreached branch and a fail-open produce the same observation unless you prove the branch was reached.* **AND THE SCOPE HALF, which is the one most likely to be skipped:** a KILLED result is a claim about one test file, but a SURVIVED result is a claim that **no test anywhere** catches the mutation — so a survivor list is only valid at the scope it was run against. Grep-by-name is not that scope: the arm script showed one consumer by name while 78 test files reach `scripts/` generically. Re-running two survivors against all 78 (on a verified-clean 1252-passed baseline) left the conclusion unchanged and the confidence entirely different. Reported by the concurrency vector against their own earlier sweep, after they had already caught the identical scope error in their checker sweep and not carried the correction back — this PR's own generator, in a reviewer's method rather than in code.

17. **BL-NEW-CRON-DRIFT-WATCHDOG-UNGUARDED** — PROPOSED 2026-08-24, a tradeoff taken deliberately in #560 and recorded so it is a decision rather than an oversight. `scripts/cron-drift-watchdog.sh` ships `100755` (fixed in #560) but is **outside every guard**: it appears in no crontab and no unit, only in `cron/README.md:94` as a copy-paste instruction. The executable-bit guard was deliberately narrowed to exclude prose sources — letting documentation put a script in scope is the "scan satisfiable by prose" category error — and the consequence is that the one script whose real-world failure was actually demonstrated (an operator following a documented line, getting a job that dies on permissions into a log nobody reads) is the one now unguarded. **The durable fix is moving that README line into `cron/gecko-alpha.crontab`**, after which the derivation covers it automatically. Out of scope for #560 because activating a cron job is an operational change, not a test change. Until then the file's mode is correct but unprotected against regression.

18. **BL-NEW-PR560-UNITS-OUTSIDE-DRIFT-WATCHDOG** — PROPOSED 2026-08-24, from the SILENT-FAILURE vector on PR #560. **The two unit files this PR adds are invisible to the one runtime instrument built to detect exactly the failure the PR's deploy notes describe.** The notes say a skipped `install` leaves `ExecStartPre` in the diff and absent from what systemd executes, "indistinguishable from a working one by any repository-level check". That sentence is true and it is the wrong frame: a **runtime** check exists — `scripts/systemd-drift-watchdog.sh` diffs repo units against `/etc/systemd/system` on a timer and pages on drift — and these two units sit outside its enumeration in **both** directions. **Direction A (repo→prod)**, `scripts/systemd-drift-watchdog.sh:113`: `find "$REPO_DIR/systemd" -maxdepth 1 …` walks `systemd/` only, while this PR's units are `scripts/recompute-coverage-watchdog.{service,timer}`. **Direction B (prod→repo)**, `DIRB_PATTERNS` at `:51-57` — `gecko-*`, `minara-*`, `systemd-drift-watchdog.*` — matches neither name, so an installed-but-stale copy is not flagged as untracked either. **Verified independently by the author (`git ls-files`): 17 unit files live in `systemd/`, exactly 2 live in `scripts/`, and they are this PR's.** The repository already encodes that units live in two places — `tests/test_watchdog_ships_executable.py:58-61` globs both `scripts/*` and `systemd/*` — so the test lists both directories while the runtime instrument lists one. That divergence is a single grep, not a subtle inference. **Fix: `git mv` both units to `systemd/` and update the install paths in the runbook and deploy notes.** That closes both directions at once, because Direction B resolves against `$REPO_DIR/systemd/$name`. **Do NOT take the one-line version.** Adding `recompute-coverage-watchdog.*` to `DIRB_PATTERNS` without moving the files makes Direction B page `UNTRACKED PROD UNIT` forever, since the name will never be found under `systemd/` — a permanent false page installed in the drift alarm that watches everything else, i.e. the cry-wolf failure this PR already rejected four separate times. **Why deferred rather than fixed in #560:** nothing is broken at runtime. The `.sh` mode bits arrive by `git pull`, so `ExecStart` works; the missing `ExecStartPre` is a guard that does not exist rather than a service that does not run. The fix also rewrites deploy instructions the ops-safety slot verified **on the box**, which is not a change to make in the last SHA before merge. This is monitoring-coverage debt the PR created; it belongs beside the probe-liveness heartbeat ticket (14) — both are "an instrument exists or should, and this subsystem is outside it".

19. **BL-NEW-UNIT-GLOB-GUARD-SELF-REFERENTIAL** — PROPOSED 2026-08-24, from the TEST-EFFICACY vector on PR #560. **`test_the_unit_scan_covers_EVERY_unit_glob` builds its expectation from the tuple it is checking**, so the assertion it wrote for itself is inert. `by_glob: dict[str, set[str]] = {g: set() for g in _UNIT_GLOBS}` — drop a glob from `_UNIT_GLOBS` and `by_glob` simply has no such key, so the `for glob, units in by_glob.items(): assert units` loop never examines it and passes. The mutant *does* die, but via `KeyError: 'scripts/*.service'` from the hardcoded anchor two lines down — an incidental crash, not the diagnostic message the test authored. **Same self-referential shape as the first tautology found on this PR** (`assert SURFACES == list(Database._RECOMPUTE_SURFACES)`, a value compared against its own definition), making this the second instance of "the expectation is derived from the subject". **One-line fix:** make `by_glob` a literal, mirroring `by_source = {"unit": set(), "crontab": set()}` in the derivation guard 80 lines up — which uses a literal and produces the clean message. **Not taken in #560, deliberately and consistently with ticket 18:** coverage is intact (the mutant dies), the complaint is that a neighbouring line does the work, and `tests/` is a WATCHED path — taking a non-blocking nit here would lapse all four mandatory clearances for a diagnostic-quality improvement. **The real risk it guards is a future edit:** if the hardcoded anchors were ever softened into a loop over `_UNIT_GLOBS`, the guard goes fully vacuous with nothing left to kill the mutant. That is why it is worth the one line, later.

20. **BL-NEW-CLEARANCE-GATE-FLAGS-DOCSTRING-ONLY-DELTAS** — PROPOSED 2026-08-24, observed live on PR #560 while collecting clearances. **The gate's tree-hash rule reports LAPSED for a change with no executable effect, and this is now demonstrated rather than theoretical.** `test-efficacy` was cleared at `54d94520`; commit `7a3f294c` (labelled `docs:`) edited a docstring in `tests/test_watchdog_ships_executable.py`; `_moved()` duly reports `tests` as differing across `54d94520..HEAD`. Proven docstring-only by AST equality after docstring-stripping: `ast.dump` identical, raw text not. So the tree-hash rule is *correct as specified* — a clearance is a property of a revision — and simultaneously produces a re-review demand that no reviewer could act on differently. **Do NOT "fix" this by making the gate AST-aware.** That means parsing every watched file on every run, and inheriting syntax errors, non-Python watched paths (`uv.lock`, `Dockerfile`, `systemd/*`, `.github/*`), and a whole new fail-open surface in the one script whose entire job is to not fail open — the exact shape of every defect this PR spent four rounds closing. **The right fix is documentation, not code:** reviewers should know that a LAPSED notice may be textual-only, and that the correct first response is to run AST equality on the moved files and re-name the SHA if it holds, rather than re-reviewing from scratch. The cost of the conservative rule is one cheap check per lapse; the cost of relaxing it is the fail-open the rule exists to prevent. **Cry-wolf risk is the thing to watch:** if lapse notices become routine for comment edits, re-clearances get rubber-stamped and the gate degrades into advisory text -- which was literally true until branch protection landed 2026-08-28 (13(d) closed); the cry-wolf risk survives the closure, because a required check that is routinely red for uninteresting reasons gets routinely overridden. Worth revisiting only if that is observed in practice — a commit convention that keeps prose out of watched paths would address it without touching the gate.

21. **BL-NEW-PER-PR-CLEARANCE-FILES** — PROPOSED 2026-08-24, from a defect found in #560 **at merge time, by CI, after five review rounds and five clearances**. **The gate and its own test-suite were MUTUALLY EXCLUSIVE and no single commit could satisfy both.** `scripts/check_reviewer_clearances.py` FAILS a branch with a watched delta unless clearances are recorded in `.reviewers.toml`; `test_the_shipped_declaration_records_no_stale_clearances` read `REPO_ROOT/.reviewers.toml` — the **working tree** — and failed the instant they were recorded. So the gate was unusable on precisely the branches it exists to police, and the contradiction was invisible until the first real declaration was committed: every prior CI run had the gate red for *lack* of clearances, which masked the fact that recording them turned a different job red. **Neither state is green, and each failure mode hides the other.** Closed in #560 by scoping the test to master **as committed** (`git show origin/master:.reviewers.toml`) rather than the working tree, which is what its own docstring always claimed it checked. **Residual this ticket exists for:** because the repo squash-merges, a cleared PR carries its clearance SHAs into master, where they are no longer ancestors — so **master must be reset to an empty `[clearances]` after every cleared merge**. That is a manual post-merge step, and a manual step that is only needed occasionally is one that will be forgotten. It is currently *detected* (the next PR's CI fails with an explicit "reset master's [clearances]" message) but not *prevented*. **Durable fix: give each PR its own file** — `.reviewers/<PR#>.toml`, with the gate reading the one matching the PR under test. Master then accumulates a per-PR historical record, nothing is ever inherited, staleness becomes structurally impossible, and the post-merge reset disappears rather than being remembered. Not taken in #560 because it changes `scripts/` and `.github/`, which would lapse every clearance for a design improvement to a gate that has no branch protection behind it anyway (ticket 13(d)). **THE DEFERRAL IS CONDITIONAL, and the condition is one line.** Deferring is safe only while the forgotten-reset failure stays LOUD. The bootstrap branch in `test_the_shipped_declaration_records_no_stale_clearances` must `pytest.skip`, never bare-`return`: a red on master has two green-making fixes, emptying `[clearances]` (correct) and DELETING `.reviewers.toml` (wrong, and green). A bare return reports PASSED while silently disabling both the test and the gate's declaration — at which point detection vanishes permanently and **this ticket collapses into ticket 18's shape** (a manual step with no observable). Both the silent-failure and logic vectors flagged that line independently. If a future edit removes the skip, 21 stops being deferrable.

22. **BL-NEW-JOINT-SATISFIABILITY-IS-A-SEPARATE-PROPERTY** — PROPOSED 2026-08-24, the general form of the defect CI found in #560 at merge time. **When two mechanisms constrain the same artefact, each being individually correct is not closure — the reachability of a jointly-satisfying state is a separate property and needs its own test.** In #560 the gate demanded clearances in `.reviewers.toml` while its own conformance test demanded that file carry none; every component test passed, every review round passed, and no commit could satisfy both. **Record why it hid, because that is the transferable part: both mechanisms were red for individually explicable reasons, and each red masked the other.** "Of course it's red, there are no clearances yet" is a complete, satisfying, and wrong explanation — and the configuration that would have exposed the contradiction was never once constructed. **Testable form — a green-is-reachable test:** build the intended good state (clearances recorded on the branch, `[clearances]` empty on master) and assert in ONE test that `check_reviewer_clearances.py` exits 0 **and** the stale-clearance test passes. Nothing in this repo asserts a green state is reachable; every test in `tests/test_reviewer_clearance_gate.py` proves the gate can say NO, and none proved it can say YES while everything else also says YES. **This is "a detector that only says clean is unproven" one level up:** you must exhibit both verdicts of the SYSTEM, not just both verdicts of each component. Note the existence proof already exists — #560's own CI run, with both jobs green simultaneously, IS the jointly-satisfying state; this ticket pins it as a regression guard rather than a one-time observation. **REASON FOR DEFERRAL, restated because the weaker one erodes under time pressure.** The original reason was "every round on this PR introduced the next defect" — true and evidence-backed, but not the strongest. The strongest is that **mutual exclusivity is a LOUD failure, not a silent one**: if the gate and its conformance test ever become jointly unsatisfiable again, the symptom is *no commit is green*, on every run, immediately. Nothing hides. **What was silent here was never the failure — it was the DIAGNOSIS.** Each red had a complete, satisfying, wrong explanation, and each masked the other's existence; five rounds passed not because nobody saw red but because everybody had a reason for it. So this ticket's test is a **diagnosis-time instrument, not a detection instrument** — it converts "why is this red?" from a judgement call into a pointer. Real value, not safety-critical, which is precisely why deferring past merge is correct. Written down because the failure mode of a vaguely-motivated ticket is a later reader seeing "proves the gate works", pulling it forward under pressure, or treating its absence as outstanding risk. **The taxonomy this PR converged on, which is the durable part:** ticket 18 — omission is *silent, forever* → must become standing; ticket 21 — omission is *loud, remedy named* → defer, conditional on the skip line; ticket 22 — omission is *loud, badly explained* → defer, it buys diagnosis not detection.


23. **BL-NEW-A-GUARD-AND-ITS-PIN-MUST-LAND-TOGETHER** — PROPOSED 2026-08-24, the closing pattern of PR #560's fourteen rounds, stated by the silent-failure vector against their own contribution. **A guard and its pin must land in the same commit — and a guard added in the same commit as a *demonstration* of why it is needed is the one most likely to go unpinned, because the demonstration reads as the evidence and evidence is not a test.** The sequence on this PR is the argument, four layers deep, each fix correct and each leaving the next layer unattacked: (1) the gate and its conformance test were mutually exclusive; (2) the fix for that reproduced the `_tree` fail-open one layer down, at the file layer instead of the ref layer; (3) the fix for *that* shipped with all three of its properties silently revertible; (4) the pin for those three stopped one line short of the flag added alongside them. **Attention follows the thing just repaired rather than the thing just added.** The repo already knows "re-attack the fixes"; this is the narrower and more actionable form. **Worked counter-example in the same round, which is why the ticket is not "pin everything":** the unpinned flag (`--full-tree`) turned out to be genuinely *unpinnable* — the call fixes `cwd=REPO_ROOT` and `REPO_ROOT` is structurally the top level, so dropping the flag changes no observable behaviour and its mutant SURVIVES **correctly**. Two vectors called it a gap (one raising severity on the false premise that pytest's invocation directory matters — it does not, `cwd` is set explicitly); a third checked reachability before calling it a gap and was right. **So the rule has a precondition: a surviving mutant is a missing pin only if the property is reachable. Otherwise the survival IS the evidence of unreachability**, and the correct response is to say so in the code — an unexplained absence of a pin reads as an oversight to the next reader and invites someone to add a test that asserts against a configuration the code cannot enter. **THE OPERATIONAL TEST — derived independently and simultaneously by two vectors (see below); phrasing kept from the one whose own severity-raise was the error:** *before calling a survivor a gap, name the configuration the code must be in for the property to matter — then check the code can enter it.* For `--full-tree` that configuration is "cwd is not the repo root", and one line (`cwd=REPO_ROOT`) says it never is. **AND "IT" IS A PAIR, which is the precise form and the half both earlier statements of this rule missed:** *reachability is not a property of the guard; it is a property of the guard AND THE ENVIRONMENT.* Stated by the vector that made the error twice on the same reasoning — once generalising from the branch where a mechanism runs to the branch that skips it, once generalising from the environment where a flag is inert to the environment where it is harmful. Both times the fix was to ask which path is actually taken. That is why the operational test must NAME the configuration rather than merely assert unreachability: "can the code enter this state?" is under-specified until you say which code and which environment, and `--full-tree` is inert in one environment and actively harmful in another **with the code unchanged**. That check is **cheaper than the reproduction that got it wrong**, which is what makes it worth encoding as a habit rather than a virtue. **And note the failure mode it guards against, because it is subtler than not-measuring:** that vector *did* run something — `git ls-tree` in a shell whose cwd was `tests/`, returning `[]`. The output was real; it was simply about the wrong subject, since the code never invokes it that way. **Evidence-shaped output about the wrong subject** is more dangerous than no evidence, because it arrives with a demonstration attached and therefore argues against looking again — the same class as a broken harness reporting "I tried to reproduce it and couldn't", and the same class as a `git diff --numstat` figure that is real but measured across the wrong range. **THE PRECONDITION CUTS BOTH WAYS, which is the half this ticket originally missed: an unreachable configuration cannot ground a pin, and it cannot ground a SAFETY CLAIM either.** Worked example, from the correction for the `--full-tree` survivor: the replacement comment closed "kept anyway because it is real defence if `cwd` or `REPO_ROOT` ever moves" — and `REPO_ROOT` can move two ways with **opposite signs**. (A) descending below the git root: plain returns empty, `--full-tree` finds the file, flag HELPS. (B) the git root ascending above `REPO_ROOT` — this checkout vendored or submoduled — the pathspec resolves against the OUTER root, misses `inner/.reviewers.toml`, and returns exit 0 empty: **the flag CAUSES the vacuous skip it was added to prevent.** Both measured in a scratch repo. So a protective claim was asserted over a hypothetical nobody had exercised, three lines below the paragraph correcting a claim for being accurate about a mechanism never reached — the second such instance *inside a correction for the previous one*. If the flag is ever touched, the fix is to make the top level explicit (`-C` / `--git-dir`), never to pick a pathspec convention: both conventions are wrong in one of the two directions. **And note the operational test was derived independently and simultaneously by two vectors** ("name the configuration the property needs, then check the code can enter it" / "name the line that would have to change for the property to matter"), which is stronger evidence it is the right check than either derivation alone. Both vectors that called the survivor a gap are named in this ticket deliberately — the rule is more useful carrying the error that produced it.

24. **BL-NEW-NO-REF-ARM-UNPINNED** — PROPOSED 2026-08-24, from the TEST-EFFICACY vector's attack on the pins added in #560. **The fourth arm of `_master_declaration_text` is unpinned:** replace `raise AssertionError("neither origin/master nor master resolves ...")` at `tests/test_reviewer_clearance_gate.py:393` with `return None` and **all 25 tests pass**, because `_patch_git` hard-codes `rev-parse` to return 0 and no test drives both refs failing. **Same shape as M16/M18 one arm over** — *"could not determine" collapsing into "carries none"* — and under the revert it collapses into a **skip**, which is green in CI, which is precisely the distinction M17 exists to defend. **Deliberately NOT taken in #560, on three checks the finder ran rather than assumed.** (a) **It is not part of the hardening**: the `for ref` loop and that final raise both pre-date `698fdab0` (verified against `698fdab0^`), so the claim that #560 pinned "all three properties" is accurate and complete *for its stated scope* — this is an older arm, not one the fix established and left open. (b) **Not reachable in CI**: `.github/workflows/test.yml` sets `fetch-depth: 0` on both jobs and explicitly runs `git fetch origin "${{ github.base_ref }}"`, so `origin/master` always resolves; the reachable case is a contributor who ran `git clone --single-branch`. (c) **The consequence is a skipped self-check, not a gate bypass** — `scripts/check_reviewer_clearances.py` is a separate file and is byte-identical since `dfab433f`, so nothing in this delta can weaken the gate's own behaviour. **Fix is a fourth test modelled exactly on the existing three:** give `_patch_git` a `rev_rc` parameter, set it nonzero, assert the raise, assert the counter moved. Take it alongside ticket 19. **Recorded alongside a verification worth keeping:** the same vector neutralised all four `seen["n"] += 1` anti-vacuity counters and got **four** failures, not three — the three new pins plus `test_a_git_TIMEOUT_is_not_reported_as_an_absent_path`. So the counters are load-bearing rather than decorative, and the timeout property is pinned as well. A guard that pins a fourth property nobody had claimed for it is the opposite of the usual finding on this PR, and worth the same care in the record.

24. **BL-NEW-STRICT-UNKNOWN-KEY-REJECTION-AT-V1** — PROPOSED 2026-08-25, from PR #564's review. **The active clearance record silently ignores unknown top-level keys**, found independently by the logic vector (`scope = "only scout/"` accepted, exit 0) and the concurrency vector (a v2-shaped record carrying `scope` / `expires_at` / `supersedes` accepted, exit 0). **Not a today-defect:** a typo in a KNOWN key fails closed — a misspelled `required` trips the narrowing guard, a misspelled `watch` trips the watch floor, a misspelled `[clearances]` yields `NO SHA RECORDED`. What is ignored is an *extra* key, which today means nothing. **It becomes the whole game at v2, and this is the mechanism the `record_version` debate was really about.** That debate ran three rounds over whether a new record must DECLARE its version, and ended when both sides ran the trace: a record omitting the field and a record declaring `record_version = 1` produce *identical* output on v2-shaped content, because declaring a version does not make the content match it. The failure is a STALE value, not an ABSENT one, and requiring presence does not touch staleness — it merely converts an honest gap into a confident false assertion. **Strict unknown-key rejection closes it automatically:** a v2-shaped record declaring 1 carries keys v1 has never heard of and is refused, with no policy on record contents required. Whoever does the v2 migration should introduce it as *the* mechanism rather than as a supplement to a version field — the version field is the label, this is the check. **Cannot be added at v1 in isolation without deciding one thing first:** whether records may carry annotation keys (a reviewer note, a ticket link) that no reader consumes. If yes, the rejection needs an explicit allow-list or a reserved namespace; if no, say so in `.reviewers/README.md` before the rejection lands, because the first record it refuses will be someone's hand-written note.

25. **BL-NEW-MUTATION-HARNESS-BASELINE-FROM-OBJECT-STORE** — PROPOSED 2026-08-25, from the silent-failure vector's own contaminated run on PR #564, reported rather than quietly discarded. **Pin mutation baselines from the git OBJECT STORE, and verify the mutant is still on disk AFTER the suite returns — not only before it.** A background mutation run was killed by a tool timeout while its inner Python survived, kept mutating and restoring the same file; the reviewer then re-pinned their baseline **from the working tree while that tree held a mutant**. The next run reported `SURVIVED` because the rival runner restored pristine code **mid-suite**, so the tests ran against the real implementation and passed. They were one step from reporting "the round-trip validation is unpinned", which was false — and their precondition check (`target == baseline` before splicing) structurally could not see it, because the contamination happened *during* pytest. **Two rules:** (a) `git cat-file blob <sha>` is immutable; a file copied aside is a value only until something else writes to the source — this is the SECOND time in one session a baseline stopped being a value, after `git checkout --` discarded an uncommitted fix mid-run, and the lesson recorded then ("copy the file aside") is demonstrably not strong enough; (b) capture the mutant's bytes and assert them after pytest returns. **Both directions are possible and this is the half people get wrong:** a rival *restoring* manufactures a false SURVIVED, a rival *splicing* manufactures a false KILLED — contaminated results are unusable either way, not merely optimistic. Standing corollary: never run two mutation harnesses against one checkout; reviewers take their own `git worktree` and namespace scratch files. **THIRD FORM, found the same day by a different vector: a restore check that NORMALISES is not a restore check.** A harness reported `restored: True` while the file was byte-different from `HEAD` — LF where git had checked out CRLF — because it compared Python-read *text*, which normalises newlines. It verified *content* identity and reported it as *restoration*; `git status` disagreed and both were right about different things. So the three forms differ in kind: `git checkout --` captures the baseline as a moving **reference**; a rival runner changes the **source** under a correctly-captured one; this one compares the wrong **representation**. The first two are about *when* the baseline was taken, the third about *what was compared* — and a rule that only says "copy the file aside" catches none of them. **Compare bytes, or ask `git status`, or both — never text.** **FOURTH FORM, and it needs no rival at all: a broken harness that fails into a LEGITIMATE exit code.** A reviewer's behavioural suite pointed at a baseline script that was never created — an earlier one-liner had died on a quoting error, unnoticed. Every probe returned `rc=2, "can't open file"`. Because **`rc=2` is a legitimate verdict elsewhere in this gate**, the output read as plausible: one probe matched its expected `rc=2` *by accident*, and a 12-cell sweep reported 6 deviations that were pure artefact. They were about to report a false regression. **The generalisable half: when a harness's failure mode collides with a real verdict of the system under test, "the numbers look reasonable" is not evidence the subject was even loaded.** Fix is a precondition assert on the subject itself — exists, size, and a hash printed at import — before any probe runs. Note this is the same failure as the rival-runner case with **no rival**: the first three forms all involve something else touching the baseline, this one is the harness never having a subject. Together the four say: verify the subject, verify the mutation, verify it survived the run, and compare bytes. **FIFTH FORM, self-reported by the logic vector: the baseline was RIGHT and the LOG LIED about it.** Their `sed` rewrote the 40-character SHA constant in the harness and missed the 8-character label in its own status line, so a run against `fb9217cc` printed `restored to e0331e40 object-store content`. `BASE_BYTES` genuinely came from `git show fb9217cc:...` and the restore was verified by byte-comparison against the correct blob -- **only the human-readable claim was wrong.** The first four forms are all "the baseline was wrong"; this is "the baseline was right and the output asserted otherwise", and it is the nastiest because of an asymmetry in *auditability*: the bytes were correct and unreadable, the label was wrong and human-readable, so the only artefact a reviewer would actually read was the false one. Note the direction -- the other forms manufacture false CONFIDENCE, this one manufactures false DOUBT, and both end with the audit trail disagreeing with reality. **Rule: a provenance string must be DERIVED from the value it describes, never maintained in parallel with it** -- print `BASE_SHA[:8]` from the same variable the bytes were fetched with. Any two representations of one fact eventually disagree, and the one nobody executes is the one that drifts. The general statement, from the vector that found it: *a log line is a claim, and claims about provenance need the same treatment as claims about behaviour* -- the recompute-don't-re-read principle the gate itself is built on, applied one level up to the tooling that checks the gate. **DETECTION PROCEDURE, the transferable output of this ticket, and it generalises past this repo: REMOVE THE SUBJECT AND RE-RUN. Any test that still passes was never testing the subject.** Cheap, universal, and it answers the question for any suite -- including the ad-hoc harnesses where this class actually bites; a live assertion answers it only for the one failure mode someone thought of. Applied to PR #564's shipped suite: with the gate removed the module could not even be COLLECTED (`FileNotFoundError`, 0 tests passed), because `_GATE = _load_gate()` runs at module scope. **That immunity is incidental and undeclared** -- it is held by the same import-time derivation added for the floors, and if those ever revert to literals the module would import fine without the gate and every subprocess test would start passing vacuously. Worth one comment at the `_GATE` import saying so. **THE PROCEDURE IS INCOMPLETE AS STATED, corrected by the concurrency vector, and the correction is what makes it safe to hand to anyone else.** "Remove the subject and re-run" needs **"and then prove the survivors could not have reached it"** bolted on. **The subject you remove has to be the subject the test actually REACHES, and in a suite with both in-process and subprocess tests those are two different subjects wearing one name.** Concretely: removing the gate from disk measures IMPORTABILITY (because `_GATE` loads at module scope); pointing `SCRIPT` at a bogus path while leaving `_GATE` cached measures SUBPROCESS-TARGET correctness. Same removal in English, two different experiments. **Without the discrimination step the procedure REPORTS VACUITY WHERE THERE IS NONE -- a false positive in the vacuity detector, which is itself a fourth-form instance: an instrument whose own failure mode is indistinguishable from a real finding.** On PR #564 the raw number was `78 failed, 9 passed`, which reads as nine vacuous tests; AST-walking those nine for call shape showed every one is an in-process test that never launches the gate, turning an alarming result into a correct one. **So the load-bearing half was never the pass/fail count -- it was classifying the survivors by whether they could have reached the removed subject at all.** **UNION STATEMENT for this whole ticket, from the same vector, and it subsumes the six forms, sf's removal procedure and ops's invent-the-mutant alike: A CLEAN RESULT IS UNINTERPRETABLE UNTIL THE INSTRUMENT HAS BEEN SHOWN CAPABLE OF PRODUCING A DIRTY ONE.** That is the same sentence as *assert which exit 2, not that it exited 2*, one level up -- the test pins which failure it saw; this pins that the harness could have seen one. **The unifying statement of all five forms: every one is an INSTRUMENT REPORTING ABOUT A SUBJECT IT NEVER REACHED.** **SEVENTH FORM, and the first where the SELECTOR is the blind part rather than the fixture or the baseline: a mutation run whose TEST SELECTION cannot observe the subject.** On PR #565 a reviewer mutated the gate to restore the deleted `REVIEWERS_PREFIX` env read and reported **SURVIVED**. The mutant was correctly planted, the baseline was correct, the subject was reached -- but their `-k` expression had been written for a different block and matched none of the terms in `test_the_ENVIRONMENT_cannot_repoint_the_declaration_prefix`, the one test that owns that property. Re-run against the owning test: `1 failed`. **Had they reported the first number they would have filed a fabricated regression against a security fix that works** -- the mirror image of a false clean, and arguably worse, because a false ALARM against a correct fix invites someone to 'fix' working code. Their statement of it is the durable one: **a survivor is only evidence when the selection can see the subject.** Distinct from the six above because nothing about the subject, the mutation or the baseline was wrong -- the *aperture* was. So the precondition list grows one row: before believing a SURVIVED, assert that the selection actually collects the test that owns the property, e.g. by running the mutant against the owning test by name and requiring it to fail there. Note this form has no analogue in the KILLED direction: a spurious kill is loud, while a spurious survival is silent and reads as a finding. **SIXTH INSTANCE, and it happened INSIDE an experiment written to test for the fourth form -- which is the strongest evidence in this ticket that naming the class does not confer immunity to it.** Testing whether the shipped suite could pass vacuously, the silent-failure vector REMOVED the gate from disk and got `0 passed`. That looked like a decisive clean result and proved a weaker thing than it appeared to: `_GATE = _load_gate()` runs at module scope, so removal broke **collection** -- the module never imported and nothing ran. The question was whether the SUBPROCESS TARGET could be wrong while the module worked; the experiment answered whether the module could import. The author's variant -- point `SCRIPT` at a bogus path and leave `_GATE` cached -- is the actual shape, and produced `78 failed, 9 passed` with every survivor AST-classified as never launching the gate. **The instrument's own failure mode (`ImportError`) produced a plausible number (`0 passed`) for a reason unrelated to what was being measured** -- the fourth form exactly, committed by the vector that had just filed it, in the experiment testing for it. Their statement of it: **knowing the class does not confer immunity to it; only the check does.** Corollary for anyone running the removal procedure above: **removal must break the SUBJECT, not the HARNESS.** If deleting the subject prevents collection, you have measured importability; re-run it by corrupting the reference the subject is reached THROUGH, leaving the harness able to run.

26. **BL-NEW-STACKED-PR-AND-BASE-EQUALS-HEAD** — PROPOSED 2026-08-25, from the logic vector's terminal review of PR #564. Three should-fix items, none blocking, all fail-closed. **(a) `base == head` switches off TWO independent guards with one argument.** Verified: `--pr 42 HEAD HEAD` on a revision that both moves production *and* deletes another PR's record exits **0** with `no delta in watched paths; no clearance required`. Unreachable in CI (the workflow hardcodes `origin/${{ github.base_ref }}`) and the module docstring already frames base currency as a caller obligation — but **the output is actively reassuring, which is the bad kind of wrong**: it does not merely fail to check, it says the check passed. One line: `if merge_base == head_sha: return 2`. Deliberately not taken in #564 because it moves `scripts/` and would lapse four clearances for an unreachable-in-CI message improvement; take it in the next PR that touches the gate. **(b) Stacked PRs.** A PR branched off another PR's branch and evaluated against `master` is flagged for "changing another PR's record" — and the only edit that turns it green is **deleting the parent's record**, which is precisely the harm the guard exists to prevent. Against the true parent (which is what `github.base_ref` supplies for a stacked PR) it is clean, so CI is unaffected; this is the local-default and mis-targeted-base case. Cheapest fix is message-only: name the stacked-branch case and say explicitly *do not delete the other record*. **(c) `git show` is not a sound content oracle.** It returns 0 and renders output for a directory, a gitlink, a `..`-path and a glob-path; `git cat-file blob` returns 128 for all four. So a directory or gitlink at the record path today reports "not valid TOML" rather than unreadable — a mislabelled verdict, not a hole, since every case fails closed. One-word fix if the precise message is wanted.

27. **BL-NEW-CLEARANCE-SWEEP-BINDS-TO-A-MUTABLE-PREFIX** -- PROPOSED 2026-08-25, found by the silent-failure vector on PR #564 (their W1), NOT BLOCKING, shipped as a ticket with BOTH vectors' readings recorded because they disagreed and the disagreement is the instructive part. **The archive sweep resolves its directory as `REPO_ROOT / _GATE.DECL_PREFIX`, and `DECL_PREFIX` is `os.environ.get("REVIEWERS_PREFIX") or ".reviewers"` evaluated at gate-import time -- so the sweep is bound to the GATE'S RUNTIME CONFIGURATION, which is a different fact from WHERE THIS REPOSITORY KEEPS ITS RECORDS.** Measured with a discriminating fixture (a record that actually exists -- with an empty corpus the true and false cases skip identically and the fixture cannot see the defect): planted `.reviewers/999.toml`; `REVIEWERS_PREFIX` unset -> PASSED (swept); `=.reviewers` -> PASSED; `=some-other-dir` -> SKIPPED `"no per-PR records shipped yet"`. **A record exists and the sweep reports that none do.** The old bug was drift between two hardcoded values; the new one is dependence on a mutable variable -- **same observable, and it arrived INSIDE THE FIX for the previous instance.** Third appearance of *"the checker looked in the wrong place and reported the corpus was empty"*. **Sharper form that makes the fix obvious:** `.reviewers/README.md` is committed at HEAD, so `d.is_dir()` is true in every correct checkout -- meaning the `not d.is_dir()` -> `pytest.skip("no per-PR records shipped yet")` branch is **unreachable when the configuration is right and reachable only when the prefix is wrong. Its message is therefore false in every case where it can fire: a skip that can only fire when it is lying.** **Fix, either -- (1) make that branch FAIL rather than skip.** The directory is committed; its absence is a broken checkout, not a bootstrap state. One line, and it closes this regardless of how the prefix is resolved (prefix-independent, which is why it is preferred). **(2) Expose `DEFAULT_DECL_PREFIX = ".reviewers"` in the gate**, with `DECL_PREFIX = os.environ.get("REVIEWERS_PREFIX") or DEFAULT_DECL_PREFIX`, and derive the sweep from `DEFAULT_DECL_PREFIX` -- keeps the anti-restatement property without the environment dependence. **Same family, one more line's worth:** `Path('C:/repo') / '/abs'` -> `C:\abs` -- pathlib **discards the left operand when the right is absolute**, so an absolute `REVIEWERS_PREFIX` makes both `REPO_ROOT / DECL_PREFIX` (the sweep) and `tmp_path / DECL_PREFIX` (the test at `:1979`) resolve outside their intended root; the second would write outside `tmp_path`. Fix (2) removes this too. **Severity: not blocking.** It cannot change any PR's verdict -- `REVIEWERS_PREFIX` is unset in CI, the shipped configuration is correct, and the gate itself is untouched; it degrades a hygiene test only. **WHY TWO VECTORS DISAGREED, which is the durable half.** The ops-safety vector probed the identical env dependency and recorded it as *not a finding*, on the grounds that both outcomes are visible skips with distinguishable messages and line numbers (`:439` "directory genuinely absent or prefix points elsewhere" vs `:448` "no records committed yet"), and CI has the variable unset. The silent-failure vector called it a finding. **Both measured correctly; they used different fixtures.** With an EMPTY corpus -- what ops tested -- the broken-prefix case and the true case are genuinely indistinguishable in kind, and the skip is honest. With a RECORD PRESENT -- what sf planted -- the sweep asserts the corpus is empty when it is not, and the skip is a lie. **The defect is invisible to any fixture that does not contain the thing being swept for**, which is this repo's standing lesson about fixtures that cannot see the defect, arriving for the third time. Consequence for whoever works this ticket: **the reproduction fixture must contain a record, or it will report the bug as absent.** W1 also becomes observable in this repo for the first time only once `.reviewers/564.toml` exists -- before that commit the corpus was empty and the bug was structurally unobservable. **RESOLVED 2026-08-25, same day: the ops-safety vector reproduced it with a record planted and WITHDREW the not-a-finding verdict.** Their measurement: `record present, bogus prefix -> SKIPPED :439 "no per-PR records shipped yet"` while `41.toml` sat in `.reviewers/`; `record present, correct prefix -> 1 passed`, the sweep actually running. Same class as F1's *"is not present in the revision under review"* -- a confident, checkable-sounding statement that is false about a file you can `ls`. **The durable lesson is theirs and it is sharper than the bug:** that vector had **quoted the suite's own docstring** -- *a fixture built to satisfy the rule cannot exercise the clause that exempts records written under a different one* (`tests/test_reviewer_clearance_gate.py:472`, shipped at `e8bbbdcb`) -- in their first report, while filing O1 against a fixture that had exactly that defect, and then built a blind fixture themselves four rounds later. **ATTRIBUTION CORRECTED 2026-08-25 and the correction makes the lesson worse, which is why it is worth the words:** the first version of this ticket credited that sentence to the reviewer. It is not theirs and it is not new -- it was written by the PR author at `e8bbbdcb`, the very first SHA sent out for review, and verified here by `git log -S` rather than by memory. So the gap is NOT the familiar one between knowing a rule abstractly and remembering to apply it. The reviewer had **read that rule, stated precisely, in the file under review, and quoted it approvingly against someone else's fixture** -- and still built a blind one of their own four rounds later. That is a far shorter distance to fail across, and therefore a far more useful warning. **Note the symmetry, since it indicts the author too: the person who WROTE the sentence then misattributed it, and the person who QUOTED it then violated it. Neither of us was helped by the rule being written down** -- and note the forensic point, because the next person reconstructing an attribution in this repo hits the same wall: **`git log -S` over the file was the ONLY instrument that could settle it.** Commit authorship metadata could not, since every commit here is the same author; the reviewer could establish the sentence was not theirs but had no way to establish whose it was -- which is the strongest available argument that a rule in prose is not a control, and that ticket 28's "derive the remedy from the failing check" is the only form of this that actually binds. So the fixture question is not "does my test pass" but **"could this fixture have shown me the failure at all"** -- and it must be asked of a NEGATIVE result with more force than a positive one, **because a negative result is SELF-RATIFYING: nothing about "no finding" prompts you to look again.** **The original phrasing here said a passing test "at least invites the question of what it proves". That is false, and this PR is the counterexample:** `test_the_deploy_substrate_DEMANDS_a_clearance` and `test_a_ROOT_FILE_change_demands_a_clearance` passed for rounds with names and docstrings asserting exactly what they did not test, and nobody asked -- two reviewers and the author included. A passing test does not invite the question; if it did, mutation testing would not need to exist. **The real asymmetry is RITUAL, not suspicion: a passing test has an established way to interrogate it -- mutate the thing it claims to defend and see whether it dies. A clean probe has none, so the question is never asked at all. We built a habit for one and not the other.** Which makes the remedy concrete rather than attitudinal: **for a negative result, INVENT THE MUTANT.** Here it was one line -- put a record in the directory -- and it would have flipped the verdict inside a minute.

28. **BL-NEW-CLEARANCE-FIRST-RED-REMEDY-PRINTED-FROM-THE-CHECK** -- PROPOSED 2026-08-25, rollout hazard identified by the author and adjudicated by the ops-safety vector on PR #564. **The first PR opened after #564 merges will see `reviewer-clearances` RED with no obvious remedy, and that red is the INTENDED FIRST STATE** -- the author's record does not exist yet and cannot exist until they write it. A reasonable person concludes the gate is broken. `.reviewers/README.md` documents the schema but not *"your check is red because you have not written your record yet, and that is correct."* **The remedy belongs in the check's own failure message, and the reason is stronger than "that is what gets read at the moment of confusion."** The README and a PR template are both **ambient text that can drift out of sync with the gate**, and this PR was bitten by exactly that FOUR times: the runbook naming `.reviewers.toml`; the sweep restating `.reviewers` as a literal; the comment asserting the `None` branch was unpinned; and a README sentence claiming an absent `record_version` means the writer *did not forget*. Every one was text NEAR the mechanism rather than DERIVED from it. **A failure message is printed from inside the branch that detected the condition -- it cannot describe a state the code is not in.** Same principle as importing `MANDATORY_WATCH` instead of restating it: the remedy should be derived from the failing check, not kept next to it. **Scope -- extend the existing `This is NOT a skip` block:** state that on a new PR this is the expected first result; name the exact path AND that it must be **COMMITTED**, since the gate reads the revision rather than the working tree (the single most likely second mistake); and print a **skeleton record generated from the live constants** -- `required` from `MANDATORY_VECTORS`, `watch` from `MANDATORY_WATCH`, `record_version` from `DEFAULT_RECORD_VERSION`. The 14-entry watch list is otherwise discoverable only by failing the narrowing check twice, and a generated skeleton can never drift the way a documented one will. **Explicitly NOT the PR template:** it is the one artefact that reaches the author before the red, but it cannot be derived from the gate, so it would become the fifth thing that drifts. Keep the README as the schema reference -- it is where someone looks *before* they are confused. **Ambient text should be the place you ARRIVE AT, never the place the remedy LIVES.** **THE LIMITING PRINCIPLE, contributed by the ops-safety vector as a critique of their OWN practice, and it is the counterweight this whole review needs: verification degrades into ritual exactly where a DERIVED CONTROL ALREADY RECOMPUTES THE SAME FACT.** That vector re-ran `_moved(sorted(MANDATORY_WATCH))` by hand on five consecutive heads to confirm no watched path had moved. All five were correct and all five were **redundant**: the clearance gate recomputes exactly that at exact-head CI, against the real head and the real record, and a record naming a lapsed SHA shows `LAPSED` there. They were doing by hand, five times, a thing the system does correctly once. **Contrast with what WAS worth verifying: the author's prose descriptions of diffs -- which caught three real errors, because a diff description is prose and prose drifts, and nothing recomputes it.** So the rule is not "measure everything" -- it is **MEASURE WHAT NOTHING ELSE RECOMPUTES.** Applied to this ticket's own subject: the reason to print the remedy from the failing branch is the same reason not to hand-check a derived guard -- **once a fact has a mechanical recomputation, human restatement of it adds drift risk and no information; where it has none, human checking is the only control there is.** **Practice note, from the same vector, on what made their three corrections possible at all:** the author sent them ticket TEXT and raw numstats to check, rather than summaries to accept. **A reviewer cannot catch a misrecording they are never shown.** Two of their three pushbacks made the record worse for them, which is the direction nobody is motivated to catch and therefore the one that has to be made structurally easy.

29. **BL-NEW-GIT-RUN-STDERR-ROUND-TRIP-IS-PLATFORM-SPLIT** -- PROPOSED 2026-08-25, from the ops-safety and logic vectors jointly on PR #564, NOT BLOCKING. **`_git_run` validates `stdout` only; `stderr` is interpolated into `_git`'s `RuntimeError` and into three `RecordUnreadable` messages without a round-trip.** The shipped docstring says so and states its own limit -- *"the limit of one reviewer's attempt, not a proof."* Two additions, neither blocking. **(a) The symptom, which is platform-split in the MIRROR IMAGE of the F1 defect.** A surrogate reaching `print` is **silent on Windows** (`sys.stdout` = cp1252 / surrogateescape -> prints a replacement character, no error) and a **`UnicodeEncodeError` on Linux CI** (`PYTHONIOENCODING=utf-8:strict` -> uncaught -> exit 1 plus traceback, i.e. a transient decode problem presenting as the code this module reserves for "this PR's clearances do not hold"). F1 was broken on Windows and fine on Linux; this is the reverse. Add one line to the scope note: *if a source is ever found, the symptom is a `UnicodeEncodeError` from `print`, visible only on a strict-stdout runner.* That turns "unvalidated" into "unvalidated, and here is the shape". **The ops vector nearly reported this as a live crash on the wrong evidence** -- their first probe printed the surrogate, observed real output, and concluded "prints fine", from a stdout configuration CI does not have; caught only by checking `sys.stdout.errors` rather than trusting the result. Third time in one session the PLATFORM was the confounder. **(b) The concrete candidate, named so the next person has something specific to try rather than a general worry.** `head` and `base` are caller-supplied, Python decodes argv with `surrogateescape`, and **`merge-base` is called without `--quiet`** -- so git echoes a bad ref straight back onto stderr, into the `RuntimeError` message, into `print()`. The logic vector measured four inputs across both the `--quiet` (`rev-parse`) and un-quieted (`merge-base`) paths -- lone surrogates, a valid non-ASCII ref, and an ASCII control -- and got `rc=2, traceback=False, UnicodeEncodeError=False` on all four, with clean verdicts even where git's stderr echoed the bad bytes. **A BOUNDED negative, and the bound is stated: that was Windows, where argv is UTF-16, so a Linux runner is not covered by it.** Cheap next step is the same four inputs on a strict-stdout Linux runner; if they stay clean the caveat narrows from "unvalidated" to "unvalidated but unreachable via argv", which is a materially different claim. **REFINEMENT 2026-08-25, arrived at through TWO wrong mechanisms before the right one, which is itself the lesson. The axis is the CHARACTER CLASS, not the platform and not the stream.** Measured 2x2 on Windows/cp1252, `python 3.14.3`, output captured through a pipe: `surrogate \udce9` -> **prints** under cp1252/surrogateescape, **RAISES** under `PYTHONIOENCODING=utf-8:strict`; valid non-cp1252 character (`\u2705`) -> **RAISES** under cp1252/surrogateescape, **prints** under utf-8/strict. **They are exact inverses.** `surrogateescape` is defined to round-trip lone surrogates back to their original bytes -- `\udce9` becomes `0xe9`, which cp1252 can represent -- so it prints; under strict UTF-8 there is no escape hatch and it raises. A valid character cp1252 simply cannot represent gets no help from `surrogateescape`, which handles lone surrogates only. So: **`surrogate -> silent on Windows, raises on Linux CI`; `valid non-Latin-1 char -> raises on Windows, silent on Linux CI`. Neither platform is safe; they are unsafe for DIFFERENT INPUTS.** **This broadens 29(b)'s reachability question materially,** because the original hunt was for *invalid UTF-8* on git's stderr -- rare and hard to construct. A **valid** non-Latin-1 character is far easier for git to emit: a CJK path component, a curly quote in a localised message, an emoji in a branch name. **And that one breaks on the OPERATOR'S machine rather than on CI** -- the opposite population from the one the original framing pointed at. **HOW BOTH OF US GOT IT WRONG, which is the transferable half:** the ops vector reported "silent on Windows" from one cell; the author then hit a real `UnicodeEncodeError` while running an unrelated script, and generalised a traceback about an EMOJI onto a finding about SURROGATES -- evidence-shaped output about a different subject, and the author's intermediate "it must be the stream, because captured stdout is strict" was ALSO wrong (a pipe reports `surrogateescape`, identical to a console; the earlier reading of `utf-8/strict` was an artefact of the author's own command setting `PYTHONIOENCODING`). **Both of us measured one cell of a 2x2 and described the whole table.** The instrument was right both times; the coverage was not -- `sys.stdout.errors` answers *how* a stream fails and cannot tell you *which inputs you have not tried*. **Two things stand independent of the mechanism and are kept verbatim.** First, **the signature does not look like a crash**: earlier `print()` calls succeed, so the log stops mid-way with no traceback context around the lines that did appear -- it reads as a hang or a truncated run, and nobody debugging that from a CI log starts by suspecting an encoding error. Second, **the rule**: when characterising an encoding failure, ask what the stream's `encoding` AND `errors` are -- and *which character class you are testing*, because the two interact and testing one proves nothing about the other.

31. **BL-NEW-WORKFLOW-DELIVERING-THE-GATE-IS-PR-EDITABLE** -- PROPOSED 2026-08-29, from the ops-safety vector on PR #565. **OPERATOR ACTION; cannot be closed from inside the repository.** The `reviewer-clearances` check is delivered by `.github/workflows/test.yml`, and for `pull_request` events GitHub runs the workflow **from the PR's own merge ref** -- so the PR being judged supplies the judge. `.github` is on the watch floor, but that does not help: the mechanism that would demand clearances for the edit **is the one being neutered.** **THREE FAIL-OPEN FAMILIES, plus the fail-CLOSED boundary, both measured:** skip the job (`if:`) -> `skipped`; `|| true` (and `||true`, `|| echo x`, `|| /bin/true`, `: python ...`, `set +e`, replacing the `run:` with `echo ok`) -> `success`; and `continue-on-error: true` -> `success`, the worst of the three because it is a **legitimate YAML key that reads as an intentional CI setting** and survives human review where a shell hack does not. Fail-CLOSED and therefore deliberately NOT defended: a bogus `runs-on` leaves the check pending (blocked); a small `timeout-minutes` yields `failure` (blocked). **Confirmed that a skipped job reports under the required context name** (`reviewer-clearances: skipped` on master's own push) -- but **NOT** that GitHub counts `skipped` as satisfying a required check: the honest probe for that IS the exploit, so it stays unverified and the `|| true` variant, which needs no such assumption, is the one to reason from. **Until 2026-08-29 the entire defence was a COMMENT reading "LOAD-BEARING, do not add `|| true`"** -- prose, in the repo whose founding thesis is that prose is not a control; `git grep -nF '|| true' -- .github/` returned exactly one hit, the sentence asking people not to write it. **WHAT SHIPPED IS DEFENCE-IN-DEPTH, NOT CLOSURE, and the test file says so in place rather than only here.** A structural guard parses the workflow and PINS resolved values instead of blocklisting patterns: exactly one job per required context across all workflow files (`ctx = name or job-id`, so an id rename AND a `name:` override AND a second colliding job all trip it); `continue-on-error` **absent** rather than not-True (the `${{ }}` form parses as a truthy STRING and defeats `!= True`); the `run:` script pinned by normalized content, which is what asserts the gate is invoked AT ALL; the `if:` pinned by equality; and `fetch-depth: 0` pinned rather than reasoned about. **The first version was a blocklist, measured at 3 of 10 real tampers** -- seven survived, six of them one-liners -- while the sentence beside it claimed it "raises a one-line tamper to a two-line one". Structural version: 15/15. **PIN, DON'T BLOCKLIST -- the bad-pattern space is unbounded, the legitimate content is a small fixed structure**, so pinning makes every tamper trip one assertion and forces every legitimate change into the diff. **EVIDENCE IT GENERALISES BEYOND ITS OWN SPEC, the strongest thing here:** the ops vector attacked the axis the whole mutation series never varied -- every mutant MODIFIED an existing attribute, none **ADDED** one. They prepended a step overwriting `scripts/check_reviewer_clearances.py` before it runs, leaving the pinned step byte-identical, `continue-on-error` absent, `if:` unchanged, job name unchanged, `fetch-depth` unchanged -- every named control passes and the gate is dead. **It was caught**, by the step-list *cardinality* assertion (`len(runners) == 1`, "an added step is where a neuter hides") rather than any content check. **Pinning cardinality as well as content is what let the guard survive an attack nobody designed it for.** **Four ways the GUARD ITSELF could have failed open, all closed, all worth carrying to any future YAML guard:** `on:` parses as the BOOLEAN `True` in YAML 1.1 (measured -- top-level keys are `['name', True, 'jobs']`), so `doc.get("on", {})` asserts nothing while reporting green; locate steps by NAME not index; assert the job lookup FOUND before asserting anything about it, or a rename makes every check vacuously true and the guard reports green **because** the tamper worked; and a missing or unparseable workflow must RAISE, never skip. `yaml.safe_load`, never `load`. **THE CLOSURE, and it is the operator's:** a repository **ruleset with `required_workflows`**, pinning the workflow at a ref the PR cannot rewrite. Classic branch protection cannot express it -- it pins the app id (15368) and the context NAME, neither of which survives a workflow edit. `gh api .../rulesets` -> `[]` (independently confirmed by two vectors), so this is **additive, not a migration**. Same family as 13(d)/13(e). The in-repo guard raises a one-line tamper to a two-line one and makes it obvious in the diff -- real value, and **not** enforcement: the same PR that edits the workflow can edit the guard, and `test` ships through the same `run:` surface. **No new dependency was needed -- `pyyaml>=6.0,<7` is already declared; adding one would have touched `pyproject.toml` and `uv.lock`, both watched, and lapsed every clearance for the sake of a test.**

30. **BL-NEW-STALE-CLEARANCE-TEST-BLOCKS-ITS-OWN-REMEDY** — OBSERVED 2026-08-24, minutes after #560 merged, on the first real firing of the guard. **`test_the_shipped_declaration_records_no_stale_clearances` reads master AS COMMITTED, so once master carries stale clearances the test is red on EVERY branch — including the branch that removes them.** The remedy PR cannot be green before merge. Confirmed live: #560's four SHAs were verified non-ancestors of master within minutes of the squash, the guard armed (it had been vacuously skipping until master first carried a declaration) and fired with the correct message naming the correct remedy. **The guard is working exactly as designed; the workflow around it is not.** This is the same mutual-exclusivity shape as the defect #560 was created to fix (ticket 22), one level up: there is a state — master carrying stale clearances — from which no single commit reaches green, because the branch's own declaration is not what the test reads. **The three exits, none free:** (a) merge the reset PR with one known-red test, which violates green-before-merge and trains exactly the rubber-stamping this design exists to prevent; (b) push the reset directly to master, which skips the PR trail on the default branch and is an operator-approval action; (c) ticket 21's per-PR files (`.reviewers/<PR#>.toml`), which dissolves the problem — master never carries a live clearance table, so there is nothing to reset and nothing to block. **This materially strengthens 21 from a convenience to a correctness fix, and it is the second time deferring 21 has cost something.** **RESOLVED 2026-08-24 by an explicitly authorized one-time direct push to master, `87604f44` (rollback provenance: `b0486dd4`).** Only `.reviewers.toml` travelled in that push; documentary changes were excluded and routed through normal PRs. The exception was authorized precisely because the alternative -- merging a PR with a known-red check -- normalises "this red is justified, merge anyway", whose distinguishing evidence lives only in human judgement. **Confirmed after the reset:** the guard now ASSERTS rather than vacuously skipping (suite `25 passed, 1 skipped` -> `26 passed`), and an unrelated branch touching only `CLAUDE.md` is no longer collaterally red. **The mechanism is confirmed end-to-end:** master's own push-CI at `b0486dd4` failed on exactly this test, with the remedy in its assertion message. **Ticket 21 is now P0 and removes the exceptional state entirely** -- with master carrying no live clearance table there is nothing to reset and nothing to block. Do not generalise the direct-push exception. Note also the sequencing trap for whoever handles this: the reset is documented in `docs/deploy_notes_pr560.md` and the merge report as a post-merge step, and both describe it as *loud if forgotten* — that remains true, but "loud" now also means "loud on the fix", which nobody anticipated when the step was written. **NUMBERING, recorded because the collision is the reason this ticket was nearly LOST rather than merely renumbered:** this was authored as ticket 25 in PR #563 and never merged. Ticket 25 was then independently taken by `BL-NEW-MUTATION-HARNESS-BASELINE-FROM-OBJECT-STORE` in #564, so #563 became unmergeable as written. It was subsequently proposed for closure *as superseded* — on the reasonable-sounding but false premise that #564 carried this history. **It did not: the content was absent from master entirely, and `.reviewers/README.md` was pointing readers at the wrong ticket 25.** Caught only by grepping master for the ticket's own slug before closing the PR. **The transferable half: 'superseded by later work' is a claim about a file, and it is checkable. Check it.** A supersession asserted from memory and a supersession verified by grep are indistinguishable right up until the content is gone.
