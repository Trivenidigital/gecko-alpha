...
def replay(conn, key):
    ...
    out["candidate"] = q1(conn, """
        ...
        ORDER BY first_seen_at LIMIT 1""", (k, k, like))

    out["first_gainers_seen"] = q1(conn, """
        ...
        GROUP BY coin_id LIMIT 1""", (k, k, like))

    out["first_trending_seen"] = q1(conn, """
        SELECT coin_id, MIN(snapshot_at) first_seen FROM trending_snapshots
        WHERE lower(coin_id) LIKE ? GROUP BY coin_id LIMIT 1""", (like,))

    out["comparisons"] = q1(conn, """
        ...
        WHERE lower(coin_id)=? OR lower(symbol)=? LIMIT 1""", (k, k))

    out["tg_alert_log"] = q1(conn, """
        SELECT COUNT(*) n, SUM(outcome='sent') sent,
               GROUP_CONCAT(DISTINCT outcome) outcomes
        FROM tg_alert_log WHERE lower(token_id)=?""", (k,))

    out["paper_trade"] = q1(conn, """
        ...
        ORDER BY opened_at LIMIT 1""", (k, k))

    out["price_cache"] = q1(conn, """
        SELECT coin_id, current_price, market_cap, updated_at FROM price_cache
        WHERE lower(coin_id)=? OR lower(coin_id) LIKE ? LIMIT 1""", (k, like))

    # Verdict skeleton the analyst fills: seen? -> stage killed
    cand = out["candidate"]
    if cand is None and out["first_gainers_seen"] is None \
            and out["first_trending_seen"] is None and out["price_cache"] is None:
        out["verdict"] = "NEVER SEEN by any table — source-coverage gap (H4)"
    elif cand is None:
        out["verdict"] = "seen by snapshots but never became a candidate — ingestion filter"
    elif isinstance(cand, sqlite3.Row) and cand["alerted_at"] is None:
