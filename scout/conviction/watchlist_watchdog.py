...
async def check_watchlist_freshness(
    ...
    run_at = await db.latest_conviction_watchlist_run_at()
    if run_at is None:
        logger.info(
    ...
    if not _alerted:
        await _alert_stale(run_at, age_min, slo, session, settings, logger)
    return "down"


async def _alert_stale(run_at, age_min, slo, session, settings, logger) -> None:
    global _alerted
    from scout import alerter  # lazy: keep aiohttp out of the import path

    body = (
        "WARNING prospective conviction watchlist STALE — builder has not written a "
        f"snapshot in {round(age_min / 60, 1)}h (SLO {slo}m). Last run: {run_at}. "
        "Check gecko-pipeline + the hourly maintenance loop."
    )
    logger.info("conviction_watchlist_alert_dispatched", run_at=run_at)
    try:
