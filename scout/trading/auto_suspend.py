...
async def _rolling_stats(
    ...
    GA-01 / Phase 6 slice 2: fabricated closes are EXCLUDED — entry-price
    bookkeeping closes (slippage 0, pnl_usd exactly $0 because no price
    source ever served the token_id). Counting them dilutes
    net_pnl/drawdown toward zero and makes a bleeding signal look
    healthier — i.e., un-suspendable for its REAL losses. 12/12
    historical `dex:` closes were this shape. The exclusion now keys on
    ``exit_provenance='entry_fallback'`` (the durable label); the GA-01
    ``exit_reason='expired_stale_no_price'`` predicate is kept as an
    OR-fallback for safety (rows closed before the provenance column, or
    any writer that missed the stamp). ``stale_snapshot`` provenance
    (stale expiry + stale-onset exits) stays counted: those marks are
    stale but market-derived.
    """
