...
| The cleared/skipped log payload does not let one line stand alone | Where the recorded population is inside the judging band AND the population doubles AND recovery collapses in the same pass, the disarm is silent — and the operator reads "discarded 0.80@30, now 70 rows" without seeing that the replacement mark is 0.04. Adding `current_rate` to that line would make the collapse visible at the one moment anyone looks. **Declined at the barrier, deliberately:** it is a one-field log improvement that would have lapsed two terminal reviewer clearances by moving production code, and the rate is already carried on the adjacent hourly `chain_identity_recompute_coverage` info line — so this is not information loss, only that the warning line read in isolation lacks it. Take it in the next change that touches this function. **Scope it correctly:** the useful field is the rate being ARMED, not the rate observed. They are the same value only because the clear is immediately followed by a write at the current rate — and if that write is starved by the 2s give-up, the table ends up with no mark at all while the observed rate is whatever it was. Logging the observed rate would then describe a mark that does not exist, which is the trap `mark_written` exists to avoid one function over. Read it from the write's return value. **And scope it as the PAIR, not as one field:** on the `incomparable_unresolved` arm the payload carries `rate_judged: True` alongside `comparison_skipped`, so a reader of that one line sees a surface reporting it is watched during a pass in which nothing was judged. Not a defect - `comparison_skipped` is explicit, and the checker reads the table rather than this field, so there is no cross-layer disagreement for a harness to paper over - but it is the same kind of one-line-read gap as the missing armed rate, in the same function. Scoped narrowly as "add current_rate" it gets fixed once and the twin survives. |
