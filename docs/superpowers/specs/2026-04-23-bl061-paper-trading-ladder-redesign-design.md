...
Under the ladder, SL is disabled once leg 1 fires — the floor (current price ≤ entry → exit) supersedes it as the sole risk cap for runners. Pre-leg-1, SL is a pure downside cap, and widening from -10% to -15% gives trades 1.5× more drawdown tolerance to reach the +25% leg-1 threshold, at which point the floor locks in at-worst-zero-giveback. Per-SL-trade tail loss does increase from -10% to -15% (50% larger per stopped trade); whether net tail risk rises or falls depends on how many trades in the -10% to -15% drawdown band recover to leg 1 and arm the floor — which the 2026-05-23 calibration review will measure. Post-leg-1, SL cannot re-trigger (floor is the operative cap), so the widening only affects pre-leg-1 trades. Like the leg thresholds, this number is speculative: if post-cutover data shows the widening is net-negative, the calibration review can revert it.
