...
Windows C: observed314251493376 bytes free; read-only host df observed14454181888 bytes free. These are planning snapshots only. No new SQLite copy or dependency environment is created now. Before later validation, measure free disk again and inventory only owned artifacts. The existing rollout preflight minimum of12GiB free remains mandatory and unchanged. The6GB validation-working headroom is only an additional working-space consideration, never a replacement for that rollout gate. At the observed14.45GB free, allocating another2.83GB copy would fall below12GiB, so do not allocate it without first budgeting recovery of sufficient space. Prefer reuse of reviewed disposable space; otherwise losslessly archive only explicitly owned superseded copies, verify decompressed whole-file hashes and recoverability before retiring their uncompressed counterparts, and retain all required immutable/live validation evidence. Recheck free space after that operation and before copying, allowing bounded WAL/temp/environment/log growth while preserving the12GiB rollout gate. If safe recovery cannot establish that budget, stop instead of lowering any gate. Do not delete another owner's files or make repeated2.8GB copies to evade a failed gate.
