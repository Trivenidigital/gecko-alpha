...
Root's current5c retry is separate and may finish while this plan is reviewed. Do not assume success, d2/master, any PID, or any branch name. Before successor rollout, root must read and attest actual clean deployed HEAD, branch/ref, source manifest, dashboard process/unit/lock, pipeline identity, schema, configuration and dependency pins. The starting release may be5c; if it is stilld2 or another state, stop and reconcile the intended transition in reviewed design. Never overwrite the running retry's helper or baseline snapshot.
