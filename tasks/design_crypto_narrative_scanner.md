...
3. **Per-tweet processing pipeline** (all within Hermes skills):
   ...
   - `narrative_alert_dispatcher` skill: writes the structured event to gecko-alpha's `narrative_alerts_inbound` table via `POST /api/narrative-alert` (HMAC-authed). Does NOT directly send TG — gecko-alpha's existing alerter handles that. **V1.1 update (2026-05-13)**: classifier emits nested `extracted_items[]` array; dispatcher fans out N POSTs (one per item). **Hermes-computed `event_id`** = `sha256(tweet_id | tweet_text_hash | ca_canonical | cashtag_canonical)` (note: cashtag included to make multi-cashtag-only items from one tweet have distinct event_ids — Finding B fix). Canonicalization rules: `cashtag.strip().upper()`; `ca` is chain-aware (`.lower()` for ETH/Base, preserved verbatim for Solana base58, preserved verbatim if chain is null — NEVER guess chain from CA shape, that's a silent-corruption trap). Idempotency key is `UNIQUE(event_id)` server-side. Failure-mode behaviors: 503 → halt + structured `narrative_dispatcher_misconfig` log + NO queue (V1.1 Path B log-only; push wire-up filed as BL-NEW-NARRATIVE-OPERATOR-ALERT-WIRE for post-activation work — Hermes-first check 2026-05-13 found no installed Telegram/Slack/Discord/webhook-out skills on srilu); 401/403/409/413 → queue current + queue all unprocessed remaining + halt (auth applies batch-globally); 400 → log + continue (per-item); other 5xx + transport exception → queue + continue. **For Solana-resolved alerts, the existing M1.5c Minara emit fires downstream automatically; for ETH/BASE alerts, no Minara command (M1.5d EVM not shipped) — operator manually trades.**
...
**Idempotency (Vector B HLD-2 fold, V1.1 update 2026-05-13):** gecko-alpha schema uses `UNIQUE(event_id)` where `event_id` is Hermes-computed `sha256(tweet_id | tweet_text_hash | ca_canonical | cashtag_canonical)`. Cashtag is included to make multi-cashtag-only items from the same tweet produce distinct event_ids (Finding B fix). Canonicalization: `cashtag.strip().upper()`; `ca` is `.lower()` for ETH/Base, preserved verbatim for Solana base58 (case-sensitive), preserved verbatim if chain is null — NEVER guess chain from CA shape. Classifier output drift on the same tweet → same event_id → duplicate insert rejected. Tweet edits (X allows 30-min edit) → different `tweet_text_hash` → different event_id → new row (intended; the operator should see the edited tweet).
...
2. **One new table** `narrative_alerts_inbound` (revised per Vector A N-2 + FC-2 + Vector B HLD-2):
   ...
   CREATE TABLE narrative_alerts_inbound (
       ...
       event_id TEXT NOT NULL UNIQUE,     -- Hermes-computed sha256(tweet_id|text_hash|ca_canonical|cashtag_canonical) per V1.1 (2026-05-13)
