...
- [x] Codex collector worker (Claude review incorporated): failing regressions, then scan checkpoints independent of
...
- [x] Codex collector worker (Claude review incorporated): idempotent crash replay and canonical projection reconciliation;
...
- [x] Codex latency worker: failing regressions then chain-qualified comparisons,
  explicit invalid clocks/censoring, honest paired summaries. Own harness/tests.
- [x] Codex: reuse watchdog infrastructure; behavior tests for disabled, fresh,
...
Read-only production verification found RH disabled with no configured RPC URL; the public RPC is reachable from that host. Live activation requires those facts,
