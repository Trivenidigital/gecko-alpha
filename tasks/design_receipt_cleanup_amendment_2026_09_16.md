...
| stall_cleanup | W recovery | `early_exit_orphan` fixture; `identity.ready` precedes `release` precedes `fault.fired`; `supervisor.stalled` triggers W; `pid_kills == [direct S, state S or R]`; `live_before_signal ≥ 1`; `kills ≥ 1`; G equals published and readiness pgid; empty |
