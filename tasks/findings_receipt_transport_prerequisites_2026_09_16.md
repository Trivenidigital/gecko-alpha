...
1. **Linux OS-native containment for PREREQ-1.** Evaluate, then pick one: a transient cgroup v2 scope using `cgroup.kill`, with recursive `cgroup.events` `populated=0` as the independent oracle (an empty root `cgroup.procs` alone is insufficient because child cgroups may retain processes); or a PID namespace (`unshare --pid --fork --kill-child`) whose teardown kills every process in the namespace regardless of session. Both make `setsid` irrelevant. The plan must name the oracle, its OS version assumptions, and what it refuses to claim.
