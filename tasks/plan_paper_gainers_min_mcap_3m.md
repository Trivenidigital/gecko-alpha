...
4. Pydantic validator: must be ≥ 0; if set, must be ≤ `PAPER_MIN_MCAP` (no upward override — that's confusing semantics).
...
@classmethod
def _validate_gainers_min_mcap(cls, v: float | None, info) -> float | None:
    if v is None:
        return v
    if v < 0:
        raise ValueError("PAPER_GAINERS_MIN_MCAP must be >= 0 or None")
    # Cross-field validation deferred to model_validator if/when needed —
    # keeping field-level simple for now. Operator must ensure
    # PAPER_GAINERS_MIN_MCAP <= PAPER_MIN_MCAP semantically.
    return v
```

### 5.2 `scout/main.py` change

Line 553–559 region — replace:
...
`tests/test_main_gainers_min_mcap_resolver.py`:
...
def test_gainers_min_mcap_falls_back_to_global_when_unset(settings_factory):
    ...
    resolved = s.PAPER_GAINERS_MIN_MCAP if s.PAPER_GAINERS_MIN_MCAP is not None else s.PAPER_MIN_MCAP
    assert resolved == 5_000_000
...
def test_gainers_min_mcap_uses_override_when_set(settings_factory):
    ...
    resolved = s.PAPER_GAINERS_MIN_MCAP if s.PAPER_GAINERS_MIN_MCAP is not None else s.PAPER_MIN_MCAP
    assert resolved == 3_000_000


def test_gainers_min_mcap_rejects_negative():
    with pytest.raises(ValueError):
        Settings(PAPER_GAINERS_MIN_MCAP=-1)
```
...
- [ ] `scout/main.py:553–559` uses resolver
- [ ] `tests/test_main_gainers_min_mcap_resolver.py` passes (3 tests)
