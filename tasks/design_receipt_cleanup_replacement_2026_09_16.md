...
3. **W output contract (5.4).** W and P never write to stdout through blocking `print`. One bounded writer serves both: diagnostics share a 64 KiB budget and are clipped beyond it (the count is reported as `diagnostics.dropped` in the WORKER line); the WORKER report line is exempt from the byte budget; every write is non-blocking and bounded by the current output deadline, which `recover_descendants` sets to its report partition `TR+9` on entry and which is one second otherwise. The full recovery record travels only in the WORKER line. Clipping preserves line boundaries: a clipped diagnostic always ends with a newline (its last kept byte is replaced by one, or it is dropped whole when no room remains), and the writer terminates any unfinished line before the next record, so the authoritative WORKER line always starts a fresh line and `parse_marker` can find it (ops residual fold, review of c72e6e55). The line is considered open after any incomplete relay, not only when the intended buffer lacked a newline, because a partial non-blocking write may have left an unterminated fragment on the pipe; it closes only once a relay completes (ops residual fold, review of e99e121d).
