...
The findings doc is the operator-facing artifact; plan + design are process docs. Findings must include:
1. Inventory of what was swept
...
5. Reproducibility (the SSH commands that produced the sweep results)
...
2. **BL-NEW-CRON-TO-SYSTEMD-TIMER** — convert 2 weekly cron entries to systemd timers. Decision-by 2026-06-14.
...
5. **BL-NEW-POLYMARKET-VERIFY** — informational; operator confirms `/opt/polymarket-ml-signal/` path validity.
6. ~~BL-NEW-APACHE-AUDIT~~ — withdrawn; Apache not installed.
...
| `cron/deploy.sh` idempotent across re-runs | awk script | shell-test on srilu (one-off operator verification post-merge) |
