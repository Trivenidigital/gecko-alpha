...
- (Optional, read-only, docs-only) `docs/superpowers/registries/signal_trust_registry.v1.json` + `scripts/validate_signal_trust_registry.mjs`
...
|---|---|---|
| Session templates (plan/design/findings/review) | none found (Skills Hub checked; no repo-specific Gecko templates) | **build from scratch** (project-specific format + operator gates) |
| Autonomous-work status report surface | none found | **build from scratch** (repo-specific: backlog + tasks + PR state) |
| Agent/role map + operator gates | none found | **build from scratch** (project-specific governance + truth sources) |
| Signal trust registry + scorecards | none found | **build from scratch** (truth stays in gecko-alpha DB; Hermes is enrichment-only) |

Awesome-hermes-agent ecosystem check: informational index; no drop-in Gecko-Alpha trust/cockpit/status surfaces found. Verdict: proceed with minimal custom docs/scripts; keep everything read-only.
