...
- [ ] Two plan reviews, design and two design reviews.
- [ ] Read-only build, tests, PR and two PR reviews; exact-head CI before merge.
- [ ] Record results and deployment ownership boundary.
...
- P4 actionability data gate: CLEARED / no immediate implementation authorized on 2026-05-26. v1 closed rows are actionable=55 (+$335.53, 43/55 wins) and exploratory=16 (-$385.63, 7/16 wins), with zero malformed stamped rows. This closes the stale row-count wait but does not authorize v2/suppression/source-quality policy work; downstream items need fresh re-scope. See `tasks/findings_actionability_gate_revalidation_2026_05_26.md`.
...
- Findings: `tasks/findings_tg_alert_qualification_baseline_2026_05_29.md` (soak gate cleared; current 14d sent avg 19.3/day; per-signal + per-corpus breakdown; operator-action input UNKNOWN).
- Design: `tasks/design_tg_alert_qualification_2026_05_29.md` (separate `/api/alert_intent` endpoint; visibility-only; default `would_dispatch_alert: false`; no response_model envelope; scarcity target 3-5/day).
