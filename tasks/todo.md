...
- [x] **BL-NEW-TG-ALERT-QUALIFICATION-DESIGN parked with dependencies**: backlog now records the hard dependency on `BL-NEW-TRACKER-COCKPIT-PROMOTION-PATH`, the promotion-soak trigger requirement, the 8-step future design checklist, and anti-scope that urgency tiers are deferred until complete-surface queue volume justifies them.
- [ ] **BL-NEW-TRACKER-COCKPIT-PROMOTION-PATH**: next implementation target. Its plan/design must pre-commit promoted-candidate universe, candidates/day volume threshold, lookback maturity, and calendar backstop before any alert-qualification design resumes.
