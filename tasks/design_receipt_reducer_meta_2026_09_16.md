...
- `INNER_SLOTS`: zero used slots; `f1` used while `f0` `UNUSED`; a used slot `MISSING`; a used slot with 63-hex; a used slot with `size: -1`; an `UNUSED` slot carrying a digest; a ninth-slot key.
