...
def log(msg, color=Colors.BLUE, bold=False):
    ...
    suffix = Colors.END
    print(f"{prefix}{color}[{datetime.now(timezone.utc).isoformat()}]{Colors.END} {msg}")
...
def load_env():
    ...
    try:
        ...
        log(f"✗ Failed to load .env: {type(e).__name__}: {str(e)[:120]}", Colors.RED)
        state.blockers.append(f"Cannot read .env: {type(e).__name__}: {str(e)[:120]}")
...
def load_kol_list():
    ...
    try:
        ...
        state.blockers.append(f"Failed to parse KOL list: {type(e).__name__}: {str(e)[:120]}")
        log(f"✗ Failed to parse KOL list: {type(e).__name__}: {str(e)[:120]}", Colors.RED)
        return []
...
def run_kol_watcher(handles, seen_ids):
    ...
    for idx, handle in enumerate(handles, 1):
        try:
            ...
            log(f"  ✗ Error processing @{handle}: {type(e).__name__}: {str(e)[:120]}", Colors.RED)
            continue
    
    log(f"📊 Summary: {len(new_tweets)} new tweets from {len(handles)} handles", Colors.BOLD)
...
def _classify_one_with_backoff(idx, tweet, total):
    ...
    """
    log(f"  📄 Tweet {idx}/{total} (@{tweet['author']}):")
        ...
        for item in event['extracted_items']:
            if item.get('extracted_ca'):
                ...
                try:
                    ...
                    log(f"  ✗ Resolution error: {type(e).__name__}: {str(e)[:120]}", Colors.RED)
                    item['resolved_coin_id'] = None
    ...
    for event in resolved_events:
        ...
        for idx, item in enumerate(items):
            ...
            try:
                ...
                log(f"  ✗ Dispatch error: {type(e).__name__}: {str(e)[:120]}", Colors.RED)
                telemetry['posts_queued'] += 1
    
    # Log telemetry
    log(f"📊 Dispatch Telemetry:", Colors.BOLD)
...
def main():
    ...
    try:
        ...
        if not check_prereqs():
            log("✗ Prerequisites check failed", Colors.RED, bold=True)
        ...
        log("📈 Cycle Statistics:", Colors.BLUE, bold=True)
        log(f"   Duration: {duration:.1f}s", Colors.BLUE, bold=True)
        log(f"   Handles scanned: {len(state.handles_scanned)}", Colors.BLUE, bold=True)
        log(f"   Tweets inspected: {state.tweets_inspected}", Colors.BLUE, bold=True)
        log(f"   New tweets: {len(state.new_tweets)}", Colors.BLUE, bold=True)
        log(f"   Alerts dispatched: {state.alerts_dispatched}", Colors.BLUE, bold=True)
        log(f"   Duplicates: {state.duplicates}", Colors.BLUE, bold=True)
        log(f"   Skips (low confidence): {state.skips}", Colors.BLUE, bold=True)
        log(f"   Speculative CAs scrubbed: {state.speculative_cas_scrubbed}", Colors.BLUE, bold=True)
        log(f"   Queue length: {state.queue_length}", Colors.BLUE, bold=True)
        
        if state.blockers:
            log("⚠️  Blockers:", Colors.RED, bold=True)
            for blocker in state.blockers:
                log(f"   • {blocker}", Colors.RED)
        else:
            log("✅ No blockers - cycle completed successfully", Colors.GREEN, bold=True)
        ...
        log(f"✗ Fatal error: {type(e).__name__}: {str(e)[:120]}", Colors.RED, bold=True)
        import traceback
        traceback.print_exc()
        sys.exit(1)
    finally:
