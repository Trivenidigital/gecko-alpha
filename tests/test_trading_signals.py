...
# ---------------- BL-060: PAPER_MIN_QUANT_SCORE gate (test #11, #12, #13) ---


def _make_settings(tmp_path, **overrides):
    """Build a Settings object with test defaults plus caller overrides."""
    defaults = dict(
        TELEGRAM_BOT_TOKEN="test",
        TELEGRAM_CHAT_ID="test",
        ANTHROPIC_API_KEY="test",
...
# Test #11 — below-threshold candidate is skipped; above-threshold is admitted.
async def test_candidate_below_min_quant_score_skipped(db, tmp_path):
    import structlog.testing

    s = _make_settings(tmp_path, PAPER_MIN_QUANT_SCORE=40)
    eng = TradingEngine(mode="paper", db=db, settings=s)

    low = _make_candidate("low-score-coin", "LOW", mcap=50_000_000)
    high = _make_candidate("high-score-coin", "HIGH", mcap=50_000_000)
    await _seed_price(db, "low-score-coin", price=1.0)
    await _seed_price(db, "high-score-coin", price=1.0)
    ...
    # Summary log must fire (threshold skip triggered it)
    summary = next(
...
async def test_default_zero_threshold_admits_all(db, tmp_path):
    """With min_quant=0, even a low quant_score passes the threshold gate."""
    s = _make_settings(tmp_path, PAPER_MIN_QUANT_SCORE=0)
    eng = TradingEngine(mode="paper", db=db, settings=s)

    token = _make_candidate("low-but-valid", "LBV", mcap=50_000_000)
    await _seed_price(db, "low-but-valid", price=1.0)

    await trade_first_signals(
        eng,
        db,
        scored_candidates=[(token, 25, ["momentum_ratio"])],
        min_mcap=5_000_000,
        settings=s,
    )

    # quant=25 >= 0 so the trade should open (other filters pass too)
    assert await _open_count(db) == 1


# Test #13 — scope-boundary regression guard.
# Setting PAPER_MIN_QUANT_SCORE=99 must NOT affect other dispatchers.
async def test_other_dispatchers_ignore_threshold(db, tmp_path):
    """Non-first-signal dispatchers must ignore PAPER_MIN_QUANT_SCORE entirely."""
    from datetime import datetime as _dt
    from datetime import timezone as _tz
    ...
    s = _make_settings(tmp_path, PAPER_MIN_QUANT_SCORE=99)
    eng = TradingEngine(mode="paper", db=db, settings=s)

    # trade_gainers — reads gainers_snapshots
    await _insert_gainer(db, "gain-coin", market_cap=10_000_000)
    await trade_gainers(eng, db, min_mcap=5_000_000, settings=s)
    assert await _open_count(db) == 1, "trade_gainers blocked by threshold (must not)"

    # trade_losers — reads losers_snapshots
    await _insert_loser(db, "lose-coin", market_cap=10_000_000)
    await trade_losers(eng, db, min_mcap=5_000_000, settings=s)
    assert await _open_count(db) == 2, "trade_losers blocked by threshold (must not)"

    # trade_trending — reads trending_snapshots + price_cache
    await _insert_trending(db, "trend-coin", market_cap_rank=100)
    await _seed_price(db, "trend-coin", price=1.0, market_cap=20_000_000)
    await trade_trending(eng, db, max_mcap_rank=1500, min_mcap=5_000_000, settings=s)
    assert await _open_count(db) == 3, "trade_trending blocked by threshold (must not)"

    # trade_volume_spikes — takes VolumeSpike list directly
    spike = VolumeSpike(
        coin_id="spike-coin",
        symbol="SPK",
        name="Spike Coin",
    ...
    await db._conn.execute(
        """INSERT INTO chain_matches
           (token_id, pipeline, pattern_id, pattern_name, steps_matched,
            total_steps, anchor_time, completed_at, chain_duration_hours,
            conviction_boost)
           VALUES (?, 'coingecko', 2, 'p2', 3, 3, ?, ?, 1.0, 10)""",
        ("chain-coin", now, now),
    )
    await db._conn.commit()
    ...
    await _seed_price(db, "pred-coin", price=1.0)
    await trade_predictions(
        eng,
        db,
        prediction_models=[pred],
