...
async def _insert_gc(conn, coin_id, early, peak_gain_pct, *, lead=2000.0, symbol=None):
    """Insert a gainers_comparisons row with `early` surfaces confirming at
    `lead` minutes (others not detected)."""
    ...
    vals = [coin_id, symbol or coin_id.upper(), coin_id, now, 0, peak_gain_pct]
